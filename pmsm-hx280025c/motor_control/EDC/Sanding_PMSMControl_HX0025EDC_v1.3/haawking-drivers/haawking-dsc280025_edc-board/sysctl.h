...
typedef enum
{
	...
	SYSCTL_PERIPH_CLK_I2S = 0x0030        //!< I2S clock
...
typedef enum
{
    ...
    SYSCTL_ACCESS_HICMASTER  = 6U    //!< HICA access to the peripheral
} SysCtl_AccessMaster;
//*****************************************************************************
//
//! The following are values that can be passed to
...
SysCtl_disableWatchdog(void)
{
    ...
    // Set the disable bit.
    //
    HWREG(WD_BASE + SYSCTL_O_WDCR) |= SYSCTL_WD_CHKBITS | SYSCTL_WDCR_WDDIS;
                                  ...
                                  SysCtl_AccessPermission permission)
{
    ...
    HWREG(PERIPHAC_BASE + (uint32_t)peripheral) =
        (HWREG(PERIPHAC_BASE + (uint32_t)peripheral) &
         ~(0x3U << (uint32_t)master)) |
        ((uint32_t)permission << (uint32_t)master);
                                  ...
                                  SysCtl_AccessMaster master)
{
    ...
    // two bits dedicated to its permission setting.
    //
    return((HWREG(PERIPHAC_BASE + (uint32_t)peripheral) >> (uint32_t)master) &
           0x3U);
}
