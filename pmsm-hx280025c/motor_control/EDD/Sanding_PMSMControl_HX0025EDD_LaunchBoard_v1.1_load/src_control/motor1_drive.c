...
//
uint32_t ABSPeriodCountPrev = 0;
uint32_t ABSPeriodCount = 0;
uint32_t ABSPeriodCountDelta = 0;
float32_t ABSPeriodCountDelta_f = 0.0f;

__interrupt  void xint3ISR(void)
{
    xint3Count++;

    ABSPeriodCountPrev = HAL_readTimerCnt(halHandle,HAL_CPU_TIMER2);
    ...
    ABSPeriodCount = HAL_readTimerCnt(halHandle,HAL_CPU_TIMER2);
    ABSPeriodCountDelta = ABSPeriodCountPrev - ABSPeriodCount;
    ABSPeriodCountDelta_f = (float32_t)ABSPeriodCountDelta/USER_SYSTEM_FREQ_Hz;
}
...
__interrupt CODE_SECTION( "ramfuncs") void motor1CtrlISR(void)
{
	...
	obj->posDeltaCount++;
	if(obj->posDeltaCount>=1000)// 1000/10000=0.1s
	{
		obj->posDeltaCount=0;
