                 ...
                 uint32_t size)
{
    ...
    for(idx = 0U; idx < size; idx++)
    {
        ...
        // Write out the data 8 bits at a time.
        //
        *(uint8 *)(dataReg) = (uint32_t)((data[idx / 2]) >> ((idx % 2UL) * 8UL));
                 ...
                 uint32_t size)
{
    ...
    for(idx = 0U; idx < size; idx++)
    {
        ...
        // Write out the data 8 bits at a time.
        //
    	*(uint8 *)(dataReg) = ((data[idx / 4]) >> ((idx % 4UL) * 8UL));
...
CAN_initRAM(uint32_t base)
{
    ...
    HWREG(base + CAN_O_RAM_INIT) = CAN_RAM_INIT_CAN_RAM_INIT |
                                    CAN_RAM_INIT_KEY;

    while(!((HWREG(base + CAN_O_RAM_INIT) & CAN_RAM_INIT_MASK) ==
...
CAN_selectClockSource(uint32_t base, CAN_ClockSource source)
{
    ...
    switch(base)
    {
        case CANA_BASE:
            ...
            HWREG(CLKCFG_BASE + SYSCTL_O_CLKSRCCTL2) |= ((uint32_t)source <<
...
CAN_startModule(uint32_t base)
{
    ...
    //
    HWREG(base + CAN_O_CTL) &= ~(CAN_CTL_INIT | CAN_CTL_CCE);
}
...
CAN_enableController(uint32_t base)
{
    ...
    //
    HWREG(base + CAN_O_CTL) &= ~CAN_CTL_INIT;
}
...
//*****************************************************************************
static __always_inline void
CAN_enableTestMode(uint32_t base, uint32_t mode)
{
    //
    // Check the arguments.
    //
    ...
    //
    HWREG(base + CAN_O_TEST) &= ~((uint32_t)CAN_TEST_SILENT |
                                   (uint32_t)CAN_TEST_LBACK |
                                   (uint32_t)CAN_TEST_EXL);

    //
    // Enable test mode and set the bits in the test register.
    //
    HWREG(base + CAN_O_CTL) |= CAN_CTL_TEST;
    HWREG(base + CAN_O_TEST) |= mode;
}
...
CAN_enableMemoryAccessMode(uint32_t base)
{
    ...
    //
    HWREG(base + CAN_O_TEST) |= CAN_TEST_RDA;
}
...
CAN_setInterruptionDebugMode(uint32_t base, bool enable)
{
    ...
    if(enable)
    {
        ...
        //
        HWREG(base + CAN_O_CTL) |= CAN_CTL_IDS;
    }
    else
    {
        //
...
CAN_enableRetry(uint32_t base)
{
    ...
    // correctly.
    //
    HWREG(base + CAN_O_CTL) &= ~CAN_CTL_DAR;
}
...
CAN_isRetryEnabled(uint32_t base)
{
    ...
    //
    return((bool)((HWREG(base + CAN_O_CTL) & CAN_CTL_DAR) != CAN_CTL_DAR));
}
...
CAN_getTxRequests(uint32_t base)
{
    ...
    //
    return(HWREG(base + CAN_O_TXRQ_21));
}
...
CAN_getNewDataFlags(uint32_t base)
{
    ...
    //
    return(HWREG(base + CAN_O_NDAT_21));
}
...
CAN_getValidMessageObjects(uint32_t base)
{
    ...
    //
    return(HWREG(base + CAN_O_MVAL_21));
}
...
CAN_getInterruptMessageSource(uint32_t base)
{
    ...
    //
    return(HWREG(base + CAN_O_IPEN_21));
}
...
CAN_getGlobalInterruptStatus(uint32_t base, uint32_t intFlags)
{
    ...
    //
    return((bool)((HWREG(base + CAN_O_GLB_INT_FLG) & intFlags) != 0U));
}
