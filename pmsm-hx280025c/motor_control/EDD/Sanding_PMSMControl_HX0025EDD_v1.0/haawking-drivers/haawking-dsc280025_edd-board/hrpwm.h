...
//*****************************************************************************
#ifdef DEBUG
static __always_inline bool HRPWM_isBaseValid(uint32_t base)
{
...
//*****************************************************************************
static __always_inline void
HRPWM_setHiResPhaseShiftOnly(uint32_t base, uint32_t hrPhaseCount)
{
    //
    // Check the arguments
    //
    ...
    //
    HWREG(base + HRPWM_O_TBPHS) = hrPhaseCount << 8U;
}
...
HRPWM_setTimeBasePeriod(uint32_t base, uint32_t periodCount)
{
    ...
    //
    HWREG(base + HRPWM_O_TBPRDHR) =  (periodCount << 8U) & 0xFFFF;
    HWREG(base + HRPWM_O_TBPRD)  = (periodCount >> 8U) & 0xFFFF;
}
...
//*****************************************************************************
static __always_inline void
HRPWM_setHiResTimeBasePeriodOnly(uint32_t base, uint32_t hrPeriodCount)
{
    //
    // Check the arguments
    //
    ...
    //
    HWREG(base + HRPWM_O_TBPRDHR) = hrPeriodCount << 8U;
}
...
HRPWM_getTimeBasePeriod(uint32_t base)
{
    ...
    //
    return (((HWREG(base + HRPWM_O_TBPRD) & 0xFFFF) << 16U) | (HWREG(base + HRPWM_O_TBPRDHR) & 0xFFFF)) >> 8U;
}
                       ...
                       HRPWM_MEPEdgeMode mepEdgeMode)
{
    ...
    //
    EALLOW;
    HWREG(base + HRPWM_O_HRCNFG) =
             ((HWREG(base + HRPWM_O_HRCNFG) & ~(0x3U << (uint32_t)channel )) |
              ((uint32_t)mepEdgeMode << (uint32_t)channel));
    EDIS;
}
                        ...
                        HRPWM_MEPCtrlMode mepCtrlMode)
{
    ...
    EALLOW;
    HWREG(base + HRPWM_O_HRCNFG) =
       ((HWREG(base + HRPWM_O_HRCNFG) & ~(0x1U << ((uint32_t)channel + 2U))) |
         ((uint32_t)mepCtrlMode << ((uint32_t)channel + 2U)));
    EDIS;
}
                                       ...
                                       HRPWM_LoadMode loadEvent)
{
    ...
    EALLOW;
    HWREG(base + HRPWM_O_HRCNFG) =
       ((HWREG(base + HRPWM_O_HRCNFG) & ~(0x3U << ((uint32_t)channel + 3U))) |
         ((uint32_t)loadEvent << ((uint32_t)channel + 3U)));
    EDIS;
}
...
HRPWM_setOutputSwapMode(uint32_t base, bool enableOutputSwap)
{
    ...
    if(enableOutputSwap)
    {
        HWREG(base + HRPWM_O_HRCNFG) |= HRPWM_HRCNFG_SWAPAB;
    }
    else
    {
        HWREG(base + HRPWM_O_HRCNFG) &= ~HRPWM_HRCNFG_SWAPAB;
...
HRPWM_setChannelBOutputPath(uint32_t base, HRPWM_ChannelBOutput outputOnB)
{
    ...
    EALLOW;
    HWREG(base + HRPWM_O_HRCNFG) =
            ((HWREG(base + HRPWM_O_HRCNFG) & ~(HRPWM_HRCNFG_SELOUTB)) |
             ((uint32_t)outputOnB << 5U));
    EDIS;
}
...
HRPWM_enableAutoConversion(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + HRPWM_O_HRCNFG) |= HRPWM_HRCNFG_AUTOCONV;
    EDIS;
}
...
HRPWM_enablePeriodControl(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + HRPWM_O_HRPCTL) |= HRPWM_HRPCTL_HRPE;
    EDIS;
}
...
HRPWM_enablePhaseShiftLoad(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + HRPWM_O_HRPCTL) |= HRPWM_HRPCTL_TBPHSHRLOADE;
    EDIS;
}
...
HRPWM_setSyncPulseSource(uint32_t base, HRPWM_SyncPulseSource syncPulseSource)
{
    ...
    {
        HWREG(base + HRPWM_O_HRPCTL) =
          ((HWREG(base + HRPWM_O_HRPCTL) &
          ~(HRPWM_HRPCTL_PWMSYNCSELX_M | HRPWM_HRPCTL_PWMSYNCSEL)) |
           ((uint32_t)syncPulseSource << 1U));
    }
    else
    {
        HWREG(base + HRPWM_O_HRPCTL) =
          ((HWREG(base + HRPWM_O_HRPCTL) & ~HRPWM_HRPCTL_PWMSYNCSELX_M) |
           ((uint32_t)syncPulseSource << HRPWM_HRPCTL_PWMSYNCSELX_S));
    }
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
HRPWM_setTranslatorRemainder(uint32_t base, uint32_t trremVal)
{
    //
    // Check the arguments
    //
    ...
    //
    EALLOW;
    HWREG(base + HRPWM_O_TRREM) = (trremVal & HRPWM_TRREM_TRREM_M);
    EDIS;
}
...
//!
//! \b Note: compCount is a 25 bit value.
                             ...
                             uint32_t compCount)
{
    ...
    if(compModule == HRPWM_COUNTER_COMPARE_A)
    {
        ...
        //
        HWREG(base + HRPWM_O_CMPA) = compCount << 7U;
    }
    else
    {
        //
        ...
        //
        HWREG(base + HRPWM_O_CMPB) = compCount << 7U;
    }
}
...
//!  - HRPWM_COUNTER_COMPARE_B - counter compare B.
//!
//! \b Note: hrCompCount is an 9-bit value.
//!
//! \return None.
//
//*****************************************************************************
                                      ...
                                      uint32_t hrCompCount)
{
    ...
    if(compModule == HRPWM_COUNTER_COMPARE_A)
    {
        ...
        // Write to CMPAHR
        //
        HWREG(base + HRPWM_O_CMPA) = (HWREG(base + HRPWM_O_CMPA) & (~HRPWM_CMPA_CMPAHR_M))|
        		                                                                     (hrCompCount << 7U);
    }
    else
    {
        ...
        // Write to CMPBHR
        //
        HWREG(base + HRPWM_O_CMPB) = (HWREG(base + HRPWM_O_CMPB) & (~HRPWM_CMPB_CMPBHR_M))|
        		                                                                      (hrCompCount << 7U);
    }
}
                             ...
                             HRPWM_CounterCompareModule compModule)
{
    ...
    if(compModule == HRPWM_COUNTER_COMPARE_A)
    {
        ...
        // Read from CMPAHR
        //
        compCount = HWREG(base + HRPWM_O_CMPA) >> 7U;
    }
    else
    {
        //
        // Read from CMPBHR
        //
        compCount = HWREG(base + HRPWM_O_CMPB) >> 7U;
    }
    return(compCount);
}
                                      ...
                                      HRPWM_CounterCompareModule compModule)
{
    ...
    if(compModule == HRPWM_COUNTER_COMPARE_A)
    {
        ...
        // Read from CMPAHR
        //
        hrCompCount = (HWREG(base + HRPWM_O_CMPA) & HRPWM_CMPA_CMPAHR_M) >> 7U;
    }
    else
    {
        //
        // Read from CMPBHR
        //
        hrCompCount = (HWREG(base + HRPWM_O_CMPB) & HRPWM_CMPB_CMPBHR_M) >> 7U;
    }
    return(hrCompCount);
}
...
HRPWM_setRisingEdgeDelay(uint32_t base, uint32_t redCount)
{
    ...
    //
    HWREG(base + HRPWM_O_DBREDHR) = redCount << 8U;
    HWREG(base + HRPWM_O_DBRED) = redCount >> 8U;
}
...
//*****************************************************************************
static __always_inline void
HRPWM_setHiResRisingEdgeDelayOnly(uint32_t base, uint32_t hrRedCount)
{
    //
    // Check the arguments
    //
    ASSERT(HRPWM_isBaseValid(base));
    ASSERT(hrRedCount < 512);

    //
    // Set the High Resolution RED (Rising Edge Delay) count only
    //
    HWREG(base + HRPWM_O_DBREDHR) = hrRedCount << 8U;
}
...
HRPWM_setFallingEdgeDelay(uint32_t base, uint32_t fedCount)
{
    ...
    //
    HWREG(base + HRPWM_O_DBFEDHR) = fedCount << 8U;
    HWREG(base + HRPWM_O_DBFED) = fedCount >> 8U;
}
...
//*****************************************************************************
static __always_inline void
HRPWM_setHiResFallingEdgeDelayOnly(uint32_t base, uint32_t hrFedCount)
{
    //
    // Check the arguments
    //
    ASSERT(HRPWM_isBaseValid(base));
    ASSERT(hrFedCount < 512U);

    //
    // Set the high resolution FED (Falling Edge Delay) count
    //
    HWREG(base + HRPWM_O_DBFEDHR) = hrFedCount << 8U;
}
...
//*****************************************************************************
static __always_inline void
HRPWM_setMEPStep(uint32_t base, uint32_t mepCount)
{
    //
    // Check the arguments
    //
    ASSERT(HRPWM_isBaseValid(base));
    ASSERT(mepCount < 512U);

    //
    // Set HRPWM MEP count
    //
    EALLOW;
    HWREG(base + HRPWM_O_HRMSTEP) =
             ((HWREG(base + HRPWM_O_HRMSTEP) & ~HRPWM_HRMSTEP_HRMSTEP_M) |
               mepCount);
    EDIS;
}
                               ...
                               HRPWM_MEPDeadBandEdgeMode mepDBEdge)
{
    ...
    //
    EALLOW;
    HWREG(base + HRPWM_O_HRCNFG2) =
       ((HWREG(base + HRPWM_O_HRCNFG2) & ~HRPWM_HRCNFG2_EDGMODEDB_M) |
         ((uint32_t)mepDBEdge));
    EDIS;
}
                                 ...
                                 HRPWM_LoadMode loadEvent)
{
    ...
    EALLOW;
    HWREG(base + HRPWM_O_HRCNFG2) =
      ((HWREG(base + HRPWM_O_HRCNFG2) & ~HRPWM_HRCNFG2_CTLMODEDBRED_M) |
       ((uint32_t)loadEvent << HRPWM_HRCNFG2_CTLMODEDBRED_S));
    EDIS;
}
...
HRPWM_setFallingEdgeDelayLoadMode(uint32_t base, HRPWM_LoadMode loadEvent)
{
    ...
    EALLOW;
    HWREG(base + HRPWM_O_HRCNFG2) =
       ((HWREG(base + HRPWM_O_HRCNFG2) & ~HRPWM_HRCNFG2_CTLMODEDBFED_M) |
        ((uint32_t)loadEvent << HRPWM_HRCNFG2_CTLMODEDBFED_S));
    EDIS;
}
