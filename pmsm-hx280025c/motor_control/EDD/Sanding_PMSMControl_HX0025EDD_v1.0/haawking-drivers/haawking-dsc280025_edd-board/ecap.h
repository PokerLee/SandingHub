...
//*****************************************************************************
#ifdef DEBUG
static __always_inline bool ECAP_isBaseValid(uint32_t base)
{
    return(
           (base == ECAP1_BASE) ||
           (base == ECAP2_BASE) ||
...
//
//*****************************************************************************
static __always_inline void ECAP_setEventPrescaler(uint32_t base,
                                          uint32_t preScalerValue)
{
    ...
    HWREG(base + ECAP_O_ECCTL1) =
                 ((HWREG(base + ECAP_O_ECCTL1) & (~ECAP_ECCTL1_PRESCALE_M)) |
...
//
//*****************************************************************************
static __always_inline void ECAP_setEventPolarity(uint32_t base,
                                         ECAP_Events event,
                                         ECAP_EventPolarity polarity)
{
    ...
    //
    HWREG(base + ECAP_O_ECCTL1) =
                         (HWREG(base + ECAP_O_ECCTL1) & ~(1U << shift)) |
...
//
//*****************************************************************************
static __always_inline void ECAP_setCaptureMode(uint32_t base,
                                       ...
                                       ECAP_Events event)
{
    ...
    //
    HWREG(base + ECAP_O_ECCTL2) =
               ((HWREG(base + ECAP_O_ECCTL2) & (~ECAP_ECCTL2_CONT_ONESHT)) |
                (uint32_t)mode);
    ...
    //
    HWREG(base + ECAP_O_ECCTL2) =
               ((HWREG(base + ECAP_O_ECCTL2) & (~ECAP_ECCTL2_STOP_WRAP_M)) |
                (((uint32_t)event) << ECAP_ECCTL2_STOP_WRAP_S ));
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_reArm(uint32_t base)
{
    ASSERT(ECAP_isBaseValid(base));
    ...
    // Write to RE-ARM bit
    //
    HWREG(base + ECAP_O_ECCTL2) |= ECAP_ECCTL2_REARM;
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_enableInterrupt(uint32_t base,
                                        uint32_t intFlags)
{
    ASSERT(ECAP_isBaseValid(base));
    ...
    //
    HWREG(base + ECAP_O_ECEINT) |= intFlags;
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_disableInterrupt(uint32_t base,
                                         uint32_t intFlags)
{
...
//
//*****************************************************************************
static __always_inline uint32_t ECAP_getInterruptSource(uint32_t base)
{
    ASSERT(ECAP_isBaseValid(base));
    ...
    //
    return(HWREG(base + ECAP_O_ECFLG) & 0xFEU);
}
...
//
//*****************************************************************************
static __always_inline bool ECAP_getGlobalInterruptStatus(uint32_t base)
{
    ASSERT(ECAP_isBaseValid(base));
    ...
    //
    return((HWREG(base + ECAP_O_ECFLG) & 0x1U) == 0x1U);
}
...
//
//*****************************************************************************
static __always_inline void ECAP_clearInterrupt(uint32_t base,
                                       uint32_t intFlags)
{
    ASSERT(ECAP_isBaseValid(base));
    ASSERT((intFlags & ~(ECAP_ISR_SOURCE_CAPTURE_EVENT_1 |
    ...
    //
    HWREG(base + ECAP_O_ECCLR) = intFlags;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_clearGlobalInterrupt(uint32_t base)
{
    ASSERT(ECAP_isBaseValid(base));
    ...
    // Write to INT bit
    //
    HWREG(base + ECAP_O_ECCLR) = ECAP_ECCLR_INT;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_forceInterrupt(uint32_t base,
                                       uint32_t intFlags)
{
    ASSERT(ECAP_isBaseValid(base));
    ASSERT((intFlags & ~(ECAP_ISR_SOURCE_CAPTURE_EVENT_1 |
    ...
    //
    HWREG(base + ECAP_O_ECFRC) = intFlags;
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_enableCaptureMode(uint32_t base)
{
    ASSERT(ECAP_isBaseValid(base));
    ...
    // Clear CAP/APWM bit
    //
    HWREG(base + ECAP_O_ECCTL2) &= ~ECAP_ECCTL2_CAP_APWM;
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_enableAPWMMode(uint32_t base)
{
    ASSERT(ECAP_isBaseValid(base));
...
//
//*****************************************************************************
static __always_inline void ECAP_enableCounterResetOnEvent(uint32_t base,
                                                  ECAP_Events event)
{
    ASSERT(ECAP_isBaseValid(base));
    ...
    //
    HWREG(base + ECAP_O_ECCTL1) |= 1U << ((2U * (uint32_t)event) + 1U);
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_disableCounterResetOnEvent(uint32_t base,
                                                   ECAP_Events event)
{
    ASSERT(ECAP_isBaseValid(base));
...
//
//*****************************************************************************
static __always_inline void ECAP_enableTimeStampCapture(uint32_t base)
{
    ASSERT(ECAP_isBaseValid(base));
    ...
    // Set CAPLDEN bit
    //
    HWREG(base + ECAP_O_ECCTL1) |= ECAP_ECCTL1_CAPLDEN;
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_disableTimeStampCapture(uint32_t base)
{
    ASSERT(ECAP_isBaseValid(base));
...
//
//*****************************************************************************
static __always_inline void ECAP_setPhaseShiftCount(uint32_t base, uint32_t shiftCount)
{
    ASSERT(ECAP_isBaseValid(base));
...
ECAP_setSyncInPulseSource(uint32_t base, ECAP_SyncInPulseSource source)
{
    ...
    //
    HWREG(base + ECAP_O_SYNCINSEL) =
            (HWREG(base + ECAP_O_SYNCINSEL) & (~ECAP_SYNCINSEL_SEL_M)) |
            ((uint32_t)source & ECAP_SYNCINSEL_SEL_M);
...
//
//*****************************************************************************
static __always_inline void ECAP_enableLoadCounter(uint32_t base)
{
    ...
    // Write to SYNCI_EN
    //
    HWREG(base + ECAP_O_ECCTL2) |= ECAP_ECCTL2_SYNCI_EN;
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_disableLoadCounter(uint32_t base)
{
...
//
//*****************************************************************************
static __always_inline void ECAP_loadCounter(uint32_t base)
{
    ASSERT(ECAP_isBaseValid(base));
    ...
    // Write to SWSYNC
    //
    HWREG(base + ECAP_O_ECCTL2) |= ECAP_ECCTL2_SWSYNC;
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_setSyncOutMode(uint32_t base,
                                       ECAP_SyncOutMode mode)
{
    ...
    //
     HWREG(base + ECAP_O_ECCTL2) =
                ((HWREG(base + ECAP_O_ECCTL2) & (~ECAP_ECCTL2_SYNCO_SEL_M)) |
                 (uint32_t)mode);
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_stopCounter(uint32_t base)
{
    ASSERT(ECAP_isBaseValid(base));
    ...
    // Clear TSCTR
    //
    HWREG(base + ECAP_O_ECCTL2) &= ~ECAP_ECCTL2_TSCTRSTOP;
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_startCounter(uint32_t base)
{
    ASSERT(ECAP_isBaseValid(base));
...
//
//*****************************************************************************
static __always_inline void ECAP_setAPWMPolarity(uint32_t base,
                                        ECAP_APWMPolarity polarity)
{
    ...
    EALLOW;
    HWREG(base + ECAP_O_ECCTL2) =
               ((HWREG(base + ECAP_O_ECCTL2) & ~ECAP_ECCTL2_APWMPOL) |
                (uint32_t)polarity);
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_setAPWMPeriod(uint32_t base, uint32_t periodCount)
{
    ASSERT(ECAP_isBaseValid(base));
...
//
//*****************************************************************************
static __always_inline void ECAP_setAPWMCompare(uint32_t base, uint32_t compareCount)
{
    ASSERT(ECAP_isBaseValid(base));
...
//
//*****************************************************************************
static __always_inline void ECAP_setAPWMShadowPeriod(uint32_t base,
...
//
//*****************************************************************************
static __always_inline void ECAP_setAPWMShadowCompare(uint32_t base,
...
//
//*****************************************************************************
static __always_inline uint32_t ECAP_getTimeBaseCounter(uint32_t base)
{
    ASSERT(ECAP_isBaseValid(base));
...
//
//*****************************************************************************
static __always_inline uint32_t ECAP_getEventTimeStamp(uint32_t base, ECAP_Events event)
{
    uint32_t count;
...
//
//*****************************************************************************
static __always_inline void ECAP_selectECAPInput(uint32_t base,
...
//
//*****************************************************************************
static __always_inline void ECAP_resetCounters(uint32_t base)
{
    ASSERT(ECAP_isBaseValid(base));
    ...
    // Write to ECCTL2
    //
    HWREG(base + ECAP_O_ECCTL2) |= ECAP_ECCTL2_CTRFILTRESET;
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline void ECAP_setDMASource(uint32_t base, ECAP_Events event)
{
    ASSERT(ECAP_isBaseValid(base));
    ...
    //
    HWREG(base + ECAP_O_ECCTL2) =
               ((HWREG(base + ECAP_O_ECCTL2) & ~ECAP_ECCTL2_DMAEVTSEL_M) |
                ((uint32_t)event << ECAP_ECCTL2_DMAEVTSEL_S));
    EDIS;
}
...
//
//*****************************************************************************
static __always_inline ECAP_Events ECAP_getModuloCounterStatus(uint32_t base)
{
    uint32_t counterStatusValue;

    ASSERT(ECAP_isBaseValid(base));

    counterStatusValue = (((HWREG(base + ECAP_O_ECCTL2) &
