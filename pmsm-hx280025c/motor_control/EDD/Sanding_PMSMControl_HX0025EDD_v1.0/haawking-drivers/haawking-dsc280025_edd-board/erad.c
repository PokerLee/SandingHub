...
ERAD_configBusComp(uint32_t base, ERAD_BusComp_Config config_params)
{
    ...
    //
    HWREG(base + ERAD_O_HWBP_CNTL) =
                   ((uint32_t)config_params.comp_mode   << ERAD_HWBP_CNTL_COMP_MODE_S) |
                   ((uint32_t)config_params.bus_sel     << ERAD_HWBP_CNTL_BUS_SEL_S)   |
                   ((uint32_t)config_params.enable_stop << ERAD_HWBP_CNTL_STOP_S)      |
                   ((uint32_t)config_params.enable_int  << ERAD_HWBP_CNTL_RTOSINT_S);
    EDIS;
}
                                 ...
                                 ERAD_Counter_Config config_params)
{
    ...
    if(config_params.event == ERAD_EVENT_NO_EVENT)
    {
        ...
        //
        HWREG(base + ERAD_O_CTM_CNTL) &= ERAD_CTM_CNTL_CNT_INP_SEL_EN;
    }
    else
    {
        //
        ...
        // select enable bit.
        //
        HWREG(base + ERAD_O_CTM_INPUT_SEL) =
                   ((uint32_t)(config_params.event << ERAD_CTM_INPUT_SEL_CNT_INP_SEL_S) &
        ...
        HWREG(base + ERAD_O_CTM_CNTL) =
                ...
                ((uint32_t)config_params.enable_int   << ERAD_CTM_CNTL_RTOSINT_S)    |
                ((uint32_t)config_params.enable_stop  << ERAD_CTM_CNTL_STOP_S)       |
                ((uint32_t)config_params.event_mode   << ERAD_CTM_CNTL_EVENT_MODE_S) |
                ((uint32_t)config_params.rst_on_match << ERAD_CTM_CNTL_RST_ON_MATCH_S);
                                  ...
                                  ERAD_Counter_Input_Event stop_event)
{
    ...
    if(config_params.event == ERAD_EVENT_NO_EVENT)
    {
        ...
        HWREG(base + ERAD_O_CTM_INPUT_SEL) =
                ((uint32_t)start_event << ERAD_CTM_INPUT_SEL_STA_INP_SEL_S);

        HWREG(base + ERAD_O_CTM_INPUT_SEL_2) |=
                ((uint32_t)stop_event << ERAD_CTM_INPUT_SEL_2_STO_INP_SEL_S);
...
{
    uint32_t bitpos;
    uint32_t mask_tmp = (uint32_t)mask;
    ...
    if(mask_tmp <= 4U)
    {
        ...
        if(enable_int)
        {
            HWREG(ERAD_GLOBAL_BASE + ERAD_O_GLBL_AND_EVENT_INT_MASK) &=
                    ~(1U << bitpos);
        }
        else
        {
    ...
    else
    {
        ...
        if(enable_int)
        {
            HWREG(ERAD_GLOBAL_BASE + ERAD_O_GLBL_OR_EVENT_INT_MASK) &=
                    ~(1U << bitpos);
        }
        else
        {
