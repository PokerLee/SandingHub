...
//
#define ERAD_getBusCompInstance(base) (1U << ((base / 0x14U - 3U) & 0x7U))
#define ERAD_getCounterInstance(base) ((1U << (((base - 0xBCU) / 0x24U) & 0x3U)) << 8U)
#define ERAD_getCRCInstance(base) (1U << (((base / 0xCU) & 0xFU) - 4U))

//
// Macro function to get the HWBP event number from the base address
//
#define ERAD_BUSCOMP_BASE_TO_EVENT(base) ((base / 0x14U - 3U) & 0x7U)
...
//
//*****************************************************************************
static __always_inline ERAD_Owner
ERAD_getOwnership(void)
{
    //
    // Read Global Owner register and return value
    //
    return((ERAD_Owner)((HWREG(ERAD_GLOBAL_BASE + ERAD_O_GLBL_OWNER) &
...
ERAD_setOwnership(ERAD_Owner owner)
{
    ...
    //
    EALLOW;
    HWREG(ERAD_GLOBAL_BASE + ERAD_O_GLBL_OWNER) =
                ((uint32_t)owner & ERAD_GLBL_OWNER_OWNER_M) << ERAD_GLBL_OWNER_OWNER_S;
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
ERAD_resetCounter(uint32_t instances)
{
    //
    // Resets the counter by writing into the global registers
    //
    EALLOW;
    HWREG(ERAD_GLOBAL_BASE + ERAD_O_GLBL_CTM_RESET) |= instances >> 8U;
    EDIS;
}
...
ERAD_enableModules(uint32_t instances)
{
    ...
    //
    EALLOW;
    HWREG(ERAD_GLOBAL_BASE + ERAD_O_GLBL_ENABLE) |= instances;
    EDIS;
}
...
ERAD_getHaltStatus(void)
{
    ...
    //
    return(HWREG(ERAD_GLOBAL_BASE + ERAD_O_GLBL_HALT_STAT));
}
...
ERAD_getEventStatus(void)
{
    ...
    // Read and return from the global register
    //
    return(HWREG(ERAD_GLOBAL_BASE + ERAD_O_GLBL_EVENT_STAT));
}
...
//*****************************************************************************
static __always_inline void
ERAD_enableNMI(uint32_t instances)
{
    EALLOW;
    HWREG(ERAD_GLOBAL_BASE + ERAD_O_GLBL_NMI_CTL) |= instances;
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
ERAD_disableNMI(uint32_t instances)
{
    EALLOW;
    HWREG(ERAD_GLOBAL_BASE + ERAD_O_GLBL_NMI_CTL) &= (~instances);
    EDIS;
...
//
//*****************************************************************************
static __always_inline ERAD_Status
ERAD_getBusCompStatus(uint32_t base)
{
    ...
    //
    return((ERAD_Status)((HWREG(base + ERAD_O_HWBP_STATUS) &
...
ERAD_clearBusCompEvent(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + ERAD_O_HWBP_CLEAR) = ERAD_HWBP_CLEAR_EVENT_CLR;
    EDIS;
}
...
ERAD_getCounterStatus(uint32_t base)
{
    ...
    //
    return((ERAD_Status)((HWREG(base + ERAD_O_CTM_STATUS) &
                            ...
                            ERAD_Counter_Input_Event reset_event)
{
    ...
    HWREG(base + ERAD_O_CTM_INPUT_SEL_2) =
                           ...
                           ((uint32_t)reset_event << ERAD_CTM_INPUT_SEL_2_RST_INP_SEL_S);
    EDIS;
}
...
ERAD_clearCounterEvent(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + ERAD_O_CTM_CLEAR) = ERAD_CTM_CLEAR_EVENT_CLEAR;
    EDIS;
}
...
ERAD_clearCounterOverflow(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + ERAD_O_CTM_CLEAR) = ERAD_CTM_CLEAR_OVERFLOW_CLEAR;
    EDIS;
}
                                 ...
                                 uint32_t options)
{
    ...
    //
    EALLOW;
    HWREG(base + ERAD_O_CTM_INPUT_COND) =
                            (HWREG(base + ERAD_O_CTM_INPUT_COND) &
                            ~(0x03U << input_type)) | (options << input_type);
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
ERAD_initCRC(uint32_t instances)
{
    //
    // Write into the CRC GLOBAL CTRL register
    //
    EALLOW;
    HWREG(ERAD_CRC_GLOBAL_BASE + ERAD_O_CRC_GLOBAL_CTRL) |= instances;
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
ERAD_enableCRC(uint32_t instances)
{
    //
    // Write into the CRC GLOBAL CTRL register
    //
    EALLOW;
    HWREG(ERAD_CRC_GLOBAL_BASE + ERAD_O_CRC_GLOBAL_CTRL) |= instances << 8;
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
ERAD_disableCRC(uint32_t instances)
{
    //
    // Write into the CRC GLOBAL CTRL register
    //
...
ERAD_setSeed(uint32_t base, uint32_t seed)
{
    ...
    //
    ASSERT((HWREG(ERAD_CRC_GLOBAL_BASE + ERAD_O_CRC_GLOBAL_CTRL) &
    ...
    //
    EALLOW;
    HWREG(base + ERAD_O_CRC_SEED) = seed;
    EDIS;
}
...
ERAD_setCRCQualifier(uint32_t base, ERAD_CRC_Qualifiers qualifier)
{
    ...
    //
    EALLOW;
    HWREG(base + ERAD_O_CRC_QUALIFIER) = qualifier;
    EDIS;
}
