...
extern unsigned int __enable_interrupts();
extern unsigned int __disable_interrupts();
...
#define INTERRUPT_CPU_RTOSINT 0x4000U   //!< CPU RTOS Interrupt
...
//
//*****************************************************************************
__attribute__((unused)) static void Interrupt_nmiHandler(void)
{
    //
...
Interrupt_enableGlobal(void)
{
    ...
    //
    return(((__enable_interrupts() & 0x8U) != 8U) ? true : false);
}
...
Interrupt_disableGlobal(void)
{
    ...
    //
    return(((__disable_interrupts() & 0x8U) != 8U) ? true : false);
}
...
Interrupt_register(uint32_t interruptNumber, void (*handler)(void))
{
    ...
    address = (uint32_t)PIEVECTTABLE_BASE +
              (((interruptNumber & 0xFFFF0000U) >> 16U)* 4U);
	if(interruptNumber == INT_RTOS)
	{
		address = (uint32_t)PIEVECTTABLE_BASE +
              (((interruptNumber & 0xFFFF0000U) >> 16U)* 4U+4U);
	}
...
//*****************************************************************************
static __always_inline void
Interrupt_enableInCPU(uint32_t cpuInterrupt)
{
    //
...
//*****************************************************************************
static __always_inline void
Interrupt_disableInCPU(uint32_t cpuInterrupt)
{
    //
...
//*****************************************************************************
static __always_inline void
Interrupt_clearACKGroup(uint32_t group)
{
    //
    // Set interrupt group acknowledge bits
    //
    HWREG(PIECTRL_BASE + PIE_O_ACK) = group;
}
