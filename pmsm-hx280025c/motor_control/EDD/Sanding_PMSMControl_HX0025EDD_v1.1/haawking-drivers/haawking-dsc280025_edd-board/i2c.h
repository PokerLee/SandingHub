...
//
#define I2C_STR_INTMASK     ((uint32_t)I2C_INT_ARB_LOST |                      \
                             (uint32_t)I2C_INT_NO_ACK |                        \
                             (uint32_t)I2C_INT_REG_ACCESS_RDY |                \
                             (uint32_t)I2C_INT_RX_DATA_RDY |                   \
                             (uint32_t)I2C_INT_TX_DATA_RDY |                   \
                             (uint32_t)I2C_INT_STOP_CONDITION |                \
                             (uint32_t)I2C_INT_ADDR_TARGET)
...
I2C_enableModule(uint32_t base)
{
    ...
    HWREG(base + I2C_O_MDR) |= I2C_MDR_IRS;
}
...
I2C_enableFIFO(uint32_t base)
{
    ...
    // Enable the FIFO.
    //
    HWREG(base + I2C_O_FFTX) |= I2C_FFTX_I2CFFEN | I2C_FFTX_TXFFRST;
    HWREG(base + I2C_O_FFRX) |= I2C_FFRX_RXFFRST;
}
                          ...
                          I2C_RxFIFOLevel rxLevel)
{
    ...
    //
    HWREG(base + I2C_O_FFTX) = (HWREG(base + I2C_O_FFTX) &
                                 (~I2C_FFTX_TXFFIL_M)) | (uint32_t)txLevel;
    HWREG(base + I2C_O_FFRX) = (HWREG(base + I2C_O_FFRX) &
                                 (~I2C_FFRX_RXFFIL_M)) | (uint32_t)rxLevel;
}
                          ...
                          I2C_RxFIFOLevel *rxLevel)
{
    ...
    //
    *txLevel = (I2C_TxFIFOLevel)(HWREG(base + I2C_O_FFTX) &
                                 I2C_FFTX_TXFFIL_M);
    *rxLevel = (I2C_RxFIFOLevel)(HWREG(base + I2C_O_FFRX) &
                                 I2C_FFRX_RXFFIL_M);
}
...
//
//*****************************************************************************
static __always_inline I2C_TxFIFOLevel
I2C_getTxFIFOStatus(uint32_t base)
{
    ...
    //
    level = ((HWREG(base + I2C_O_FFTX) & I2C_FFTX_TXFFST_M) >>
              I2C_FFTX_TXFFST_S);
...
//
//*****************************************************************************
static __always_inline I2C_RxFIFOLevel
I2C_getRxFIFOStatus(uint32_t base)
{
    ...
    // Get the current FIFO status
    //
    level = ((HWREG(base + I2C_O_FFRX) & I2C_FFRX_RXFFST_M) >>
              I2C_FFRX_RXFFST_S);
...
//!
//! \return Returns the I2C prescaler(I2CPSC) cast as an uint32_t.
//
//*****************************************************************************
...
//*****************************************************************************
static __always_inline void
I2C_setTargetAddress(uint32_t base, uint32_t targetAddr)
{
    //
    // Check the arguments.
    //
    ...
    HWREG(base + I2C_O_TAR) = targetAddr;
}
...
//*****************************************************************************
static __always_inline void
I2C_setOwnAddress(uint32_t base, uint32_t Addr)
{
    //
    // Check the arguments.
    //
...
I2C_isBusBusy(uint32_t base)
{
    ...
    return((HWREG(base + I2C_O_STR) & I2C_STR_BB) == I2C_STR_BB);
}
...
//*****************************************************************************
static __always_inline void
I2C_clearStatus(uint32_t base, uint32_t stsFlags)
{
    //
    // Check the arguments.
    //
...
I2C_setConfig(uint32_t base, uint32_t config)
{
    ...
    //
    HWREG(base + I2C_O_MDR) = (HWREG(base + I2C_O_MDR) &
...
I2C_setBitCount(uint32_t base, I2C_BitCount size)
{
    ...
    // Write the selected options to the mode register.
    //
    HWREG(base + I2C_O_MDR) = (HWREG(base + I2C_O_MDR) & ~I2C_MDR_BC_M) |
                               (uint32_t)size;
}
...
I2C_sendStartCondition(uint32_t base)
{
    ...
    //
    HWREG(base + I2C_O_MDR) |= I2C_MDR_STT;
}
...
I2C_sendStopCondition(uint32_t base)
{
    ...
    //
    HWREG(base + I2C_O_MDR) |= I2C_MDR_STP;
}
...
I2C_sendNACK(uint32_t base)
{
    ...
    //
    HWREG(base + I2C_O_MDR) |= I2C_MDR_NACKMOD;
}
...
I2C_getStopConditionStatus(uint32_t base)
{
    ...
    //
    return((HWREG(base + I2C_O_MDR) & I2C_MDR_STP) != 0U);
}
...
//*****************************************************************************
static __always_inline void
I2C_setDataCount(uint32_t base, uint32_t count)
{
    //
    // Check the arguments.
    //
...
I2C_setAddressMode(uint32_t base, I2C_AddressMode mode)
{
    ...
    //
    HWREG(base + I2C_O_MDR) = (HWREG(base + I2C_O_MDR) & ~I2C_MDR_XA) |
                               (uint32_t)mode;
}
...
I2C_setEmulationMode(uint32_t base, I2C_EmulationMode mode)
{
    ...
    //
    HWREG(base + I2C_O_MDR) = (HWREG(base + I2C_O_MDR) & ~I2C_MDR_FREE) |
                               (uint32_t)mode;
}
...
I2C_enableLoopback(uint32_t base)
{
    ...
    //
    HWREG(base + I2C_O_MDR) |= I2C_MDR_DLB;
}
...
//
//*****************************************************************************
static __always_inline I2C_InterruptSource
I2C_getInterruptSource(uint32_t base)
{
    ...
    //
    return((I2C_InterruptSource)(HWREG(base + I2C_O_ISRC) &
                                 I2C_ISRC_INTCODE_M));
}
...
I2C_setExtendedMode(uint32_t base, I2C_ExtendedMode mode)
{
    ...
    //
    HWREG(base + I2C_O_EMDR) =
   (HWREG(base + I2C_O_EMDR) & ~(I2C_EMDR_BC | I2C_EMDR_FCM)) |
   (uint32_t)mode;
}
