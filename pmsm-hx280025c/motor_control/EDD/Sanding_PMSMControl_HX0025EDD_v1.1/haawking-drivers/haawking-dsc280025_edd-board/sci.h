...
SCI_setParityMode(uint32_t base, SCI_ParityType parity)
{
    ...
    HWREG(base + SCI_O_CCR) = ((HWREG(base + SCI_O_CCR) &
                                 ~(SCI_CONFIG_PAR_MASK)) | (uint32_t)parity);
}
...
//
//*****************************************************************************
static __always_inline SCI_ParityType
SCI_getParityMode(uint32_t base)
{
    ...
    //
    parity = (HWREG(base + SCI_O_CCR) & (SCI_CONFIG_PAR_MASK));
...
SCI_setAddrMultiProcessorMode(uint32_t base)
{
    ...
    //
    HWREG(base + SCI_O_CCR) |= SCI_CCR_ADDRIDLE_MODE;
}
...
SCI_lockAutobaud(uint32_t base)
{
    ...
    //
    HWREG(base + SCI_O_HBAUD) = 0x0U;
    HWREG(base + SCI_O_LBAUD) = 0x1U;

    //
    // Prepare for autobaud detection.
    // Set the CDC bit to enable autobaud detection and clear the ABD bit.
    //
    HWREG(base + SCI_O_FFCT) |= SCI_FFCT_CDC;
    HWREG(base + SCI_O_FFCT) |= SCI_FFCT_ABDCLR;

    //
    // Wait until we correctly read an 'A' or 'a' and lock
    //
    while((HWREG(base + SCI_O_FFCT) & SCI_FFCT_ABD) != SCI_FFCT_ABD)
    {
    }
                          ...
                          SCI_RxFIFOLevel rxLevel)
{
    ...
    //
    HWREG(base + SCI_O_FFTX) = (HWREG(base + SCI_O_FFTX) &
                                 (~SCI_FFTX_TXFFIL_M)) | (uint32_t)txLevel;
    HWREG(base + SCI_O_FFRX) = (HWREG(base + SCI_O_FFRX) &
                                 (~SCI_FFRX_RXFFIL_M)) | (uint32_t)rxLevel;
}
                          ...
                          SCI_RxFIFOLevel *rxLevel)
{
    ...
    //
    *txLevel = (SCI_TxFIFOLevel)(HWREG(base + SCI_O_FFTX) &
                                 SCI_FFTX_TXFFIL_M);
    *rxLevel = (SCI_RxFIFOLevel)(HWREG(base + SCI_O_FFRX) &
                                 SCI_FFRX_RXFFIL_M);
}
              ...
              uint32_t *config)
{
    ...
    //
    *baud = lspclkHz /
            ((1U + (uint32_t)((uint32_t)(HWREG(base + SCI_O_HBAUD) << 8U) |
               HWREG(base + SCI_O_LBAUD))) * 8U);

    //
    // Get the parity, data length, and number of stop bits.
    //
    *config = HWREG(base + SCI_O_CCR) & (SCI_CONFIG_PAR_MASK |
...
SCI_enableModule(uint32_t base)
{
    ...
    //
    HWREG(base + SCI_O_CTL1) |= (SCI_CTL1_TXENA | SCI_CTL1_RXENA |
                                  SCI_CTL1_SWRESET);
}
...
SCI_disableModule(uint32_t base)
{
    ...
    // Disable the FIFO.
    //
    HWREG(base + SCI_O_FFTX) &= ~(SCI_FFTX_SCIFFENA);
...
SCI_enableTxModule(uint32_t base)
{
    ...
    //
    HWREG(base + SCI_O_CTL1) |= (SCI_CTL1_TXENA | SCI_CTL1_SWRESET);
}
...
SCI_disableTxModule(uint32_t base)
{
    ...
    // Disable TX.
    //
    HWREG(base + SCI_O_CTL1) &= ~SCI_CTL1_TXENA;
}
...
SCI_enableRxModule(uint32_t base)
{
    ...
    //
    HWREG(base + SCI_O_CTL1) |= (SCI_CTL1_RXENA | SCI_CTL1_SWRESET);
}
...
SCI_disableRxModule(uint32_t base)
{
    ...
    // Disable RX.
    //
    HWREG(base + SCI_O_CTL1) &= ~SCI_CTL1_RXENA;
}
...
SCI_enableSleepMode(uint32_t base)
{
    ...
    // Set sleep bit
    //
    HWREG(base + SCI_O_CTL1) |= SCI_CTL1_SLEEP;
}
...
SCI_enableFIFO(uint32_t base)
{
    ...
    // Enable the FIFO.
    //
    HWREG(base + SCI_O_FFTX) |= SCI_FFTX_SCIRST;
    HWREG(base + SCI_O_FFTX) |= SCI_FFTX_SCIFFENA | SCI_FFTX_TXFIFORESET;
    HWREG(base + SCI_O_FFRX) |= SCI_FFRX_RXFIFORESET;
}
...
SCI_isFIFOEnabled(uint32_t base)
{
    ...
    //
    return(((HWREG(base + SCI_O_FFTX) & SCI_FFTX_SCIFFENA) ==
...
SCI_resetTxFIFO(uint32_t base)
{
    ...
    // Reset the specified FIFO.
    //
    HWREG(base + SCI_O_FFTX) &= ~SCI_FFTX_TXFIFORESET;
    HWREG(base + SCI_O_FFTX) |= SCI_FFTX_TXFIFORESET;
}
...
SCI_isDataAvailableNonFIFO(uint32_t base)
{
    ...
    //
    return(((HWREG(base + SCI_O_RXST) & SCI_RXST_RXRDY) ==
...
SCI_isSpaceAvailableNonFIFO(uint32_t base)
{
    ...
    //
    return(((HWREG(base + SCI_O_CTL2) & SCI_CTL2_TXRDY) ==
...
//
//*****************************************************************************
static __always_inline SCI_TxFIFOLevel
SCI_getTxFIFOStatus(uint32_t base)
{
    ...
    //
    return((SCI_TxFIFOLevel)((HWREG(base + SCI_O_FFTX) & SCI_FFTX_TXFFST_M) >>
                             SCI_FFTX_TXFFST_S));
}
...
//
//*****************************************************************************
static __always_inline SCI_RxFIFOLevel
SCI_getRxFIFOStatus(uint32_t base)
{
    ...
    // Get the current FIFO status
    //
    return((SCI_RxFIFOLevel)((HWREG(base + SCI_O_FFRX) & SCI_FFRX_RXFFST_M) >>
                             SCI_FFRX_RXFFST_S));
}
...
SCI_isTransmitterBusy(uint32_t base)
{
    ...
    if(SCI_isFIFOEnabled(base))
    {
        ...
        //
        return(((HWREG(base + SCI_O_FFTX) & SCI_FFTX_TXFFST_M) !=
                 0) ? true : false);
    }
    else
    {
        ...
        //
        return(((HWREG(base + SCI_O_CTL2) & SCI_CTL2_TXEMPTY) ==
...
//*****************************************************************************
static __always_inline void
SCI_writeCharBlockingFIFO(uint32_t base, uint32_t data)
{
    //
    // Check the arguments.
    //
...
//*****************************************************************************
static __always_inline void
SCI_writeCharBlockingNonFIFO(uint32_t base, uint32_t data)
{
    //
    // Check the arguments.
    //
...
//*****************************************************************************
static __always_inline void
SCI_writeCharNonBlocking(uint32_t base, uint32_t data)
{
    //
    // Check the arguments.
    //
...
SCI_readCharBlockingFIFO(uint32_t base)
{
    ...
    //
    return(uint32_t)(HWREG(base + SCI_O_RXBUF) & SCI_RXBUF_SAR_M);
}
...
//! the original API to this API.
//!
//! \return Returns \e uint32_t which is read from the receive buffer.
//
//*****************************************************************************
static __always_inline uint32_t
...
SCI_enableLoopback(uint32_t base)
{
    ...
    //
    HWREG(base + SCI_O_CCR) |= SCI_CCR_LOOPBKENA;
}
...
SCI_getOverflowStatus(uint32_t base)
{
    ...
    //
    return((HWREG(base + SCI_O_FFRX) & SCI_FFRX_RXFFOVF) == SCI_FFRX_RXFFOVF);
}
...
SCI_clearOverflowStatus(uint32_t base)
{
    ...
    //
    HWREG(base + SCI_O_FFRX) |= SCI_FFRX_RXFFOVRCLR;
}
