...
{
    float32_t Kp;                //!< PI控制器的比例增益
    float32_t Ki;                //!< PI控制器的积分增益
    float32_t Error;			 //!< PI控制器的误差
    float32_t Ui;                //!< PI控制器的积分器起始值
    float32_t refValue;          //!< PI控制器的参考输入值
    float32_t fbackValue;        //!< PI控制器的反馈输入值
    float32_t ffwdValue;         //!< PI控制器的前馈输入值
    float32_t outMin;            //!< PI控制器允许的最小输出值
    float32_t outMax;            //!< PI控制器允许的最大输出值
} PI_Obj;
...
static __UNUSED ALWAYSINLINE void
PI_runSpeed(PI_Handle handle, const float32_t refValue,
       const float32_t fbackValue, float32_t *pOutValue,
	   uint32_t direction, float32_t compensate, float32_t threshold)
{
    float32_t Kp = PI_getKp(handle);
    float32_t Ki = PI_getKi(handle);
    float32_t Up;
    ...
    float32_t outMax = PI_getOutMax(handle);
    float32_t outMin = PI_getOutMin(handle);
    float32_t tempCompensate = (float32_t)direction * compensate;
    ...
    // Compute the integral output with saturation
    if(fabsf(fbackValue) < threshold)
    {
    	Ui += Ki *  handle->Error + tempCompensate;
    }
    else
    {
    	Ui += Ki *  handle->Error;
