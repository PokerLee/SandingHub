...
void HAL_init(void *pMemory)
{
    ...
    obj->linHandle = LINA_BASE;             //!< the SCIA handle
...
//    ADC_setSOCPriority(PFC_IAC_ADC_BASE, ADC_PRI_SOC0_HIPRI);
    ...
    // ISEN_A_M1
    ADC_setupSOC(MTR1_IU_ADC_BASE, ADC_SOC_NUMBER0, MTR1_ADC_TIGGER_SOC,
    ...
    // ISEN_B_M1
    ADC_setupSOC(MTR1_IV_ADC_BASE, ADC_SOC_NUMBER0  , MTR1_ADC_TIGGER_SOC,
    ...
    // ISEN_C_M1
    ADC_setupSOC(MTR1_IW_ADC_BASE, ADC_SOC_NUMBER2  , MTR1_ADC_TIGGER_SOC,
    ...
    // VSEN_A_M1
    ADC_setupSOC(MTR1_VU_ADC_BASE, ADC_SOC_NUMBER2  , MTR1_ADC_TIGGER_SOC,
    ...
    // VSEN_B_M1
    ADC_setupSOC(MTR1_VV_ADC_BASE, ADC_SOC_NUMBER4, MTR1_ADC_TIGGER_SOC,
    ...
    // VSEN_C_M1
	ADC_setupSOC(MTR1_VW_ADC_BASE, ADC_SOC_NUMBER4  , MTR1_ADC_TIGGER_SOC,
	...
	ADC_setupSOC(MTR1_VDC_ADC_BASE, ADC_SOC_NUMBER6 , MTR1_ADC_TIGGER_SOC,
    ...
    // M2
    // ISEN_A_M2
    ADC_setupSOC(MTR2_IU_ADC_BASE, ADC_SOC_NUMBER6, MTR2_ADC_TIGGER_SOC,
    ...
    // ISEN_B_M2
    ADC_setupSOC(MTR2_IV_ADC_BASE, ADC_SOC_NUMBER10, MTR2_ADC_TIGGER_SOC,
    ...
    // ISEN_C_M2
    ADC_setupSOC(MTR2_IW_ADC_BASE, ADC_SOC_NUMBER8, MTR2_ADC_TIGGER_SOC,
    ...
    // VSEN_A_M2
    ADC_setupSOC(MTR2_VU_ADC_BASE, ADC_SOC_NUMBER8, MTR2_ADC_TIGGER_SOC,
                 MTR2_VU_ADC_CH_NUM, MTR2_ADC_V_SAMPLEWINDOW);

    ADC_setupSOC(MTR2_VU_ADC_BASE, MTR2_VU_ADC_SOC_NUM, MTR2_ADC_TIGGER_SOC,
                 MTR2_VU_ADC_CH_NUM, MTR2_ADC_V_SAMPLEWINDOW);

    // VSEN_B_M2
    ADC_setupSOC(MTR2_VV_ADC_BASE, ADC_SOC_NUMBER10, MTR2_ADC_TIGGER_SOC,
                 MTR2_VV_ADC_CH_NUM, MTR2_ADC_V_SAMPLEWINDOW);

    ADC_setupSOC(MTR2_VV_ADC_BASE, MTR2_VV_ADC_SOC_NUM, MTR2_ADC_TIGGER_SOC,
                 MTR2_VV_ADC_CH_NUM, MTR2_ADC_V_SAMPLEWINDOW);

    // VSEN_C_M2
    ADC_setupSOC(MTR2_VW_ADC_BASE, ADC_SOC_NUMBER12, MTR2_ADC_TIGGER_SOC,
                 MTR2_VW_ADC_CH_NUM, MTR2_ADC_V_SAMPLEWINDOW);

    ADC_setupSOC(MTR2_VW_ADC_BASE, MTR2_VW_ADC_SOC_NUM, MTR2_ADC_TIGGER_SOC,
                 MTR2_VW_ADC_CH_NUM, MTR2_ADC_V_SAMPLEWINDOW);

    // VSEN_DCBUS
    ADC_setupSOC(MTR2_VDC_ADC_BASE, ADC_SOC_NUMBER12, MTR2_ADC_TIGGER_SOC,
...
void HAL_setupGPIOs()
{
	...
	// M1 ABS Encoder Configuration
	{
		...
		GPIO_setInterruptPin(40, GPIO_INT_XINT3);
...
void HAL_setupLIN(HAL_Handle halHandle)
{
    ...
    //
    LIN_disableModule(obj->linHandle);
    LIN_enableModule(obj->linHandle);

	//
	// Enter LIN reset state to perform configurations
	//
	LIN_enterSoftwareReset(obj->linHandle);

    //
    // Set SCI interrupts to disabled
    //
    LIN_disableSCIInterrupt(obj->linHandle, LIN_SCI_INT_ALL);

	//
	// Switch LIN into SCI mode
	//
	LIN_enableSCIMode(obj->linHandle);

	//
	// Set the SCI communication mode to idle line
	//
	LIN_setSCICommMode(obj->linHandle, LIN_COMM_SCI_IDLELINE);

	//
	// Module set to complete operations when halted by debugger
	//
	LIN_setDebugSuspendMode(obj->linHandle, LIN_DEBUG_COMPLETE);

	//
	// Disable Internal Loopback mode
	//
	LIN_disableIntLoopback(obj->linHandle);

	//
	// Set SCI to transmit one stop bit
	//
	LIN_setSCIStopBits(obj->linHandle,LIN_SCI_STOP_ONE);

	//
	// Disable parity check
	//
	LIN_disableSCIParity(obj->linHandle);

    //
    // Disable multi-buffer mode
    //
	LIN_enableMultibufferMode(obj->linHandle);
    ...
    //
    LIN_enableDataTransmitter(obj->linHandle);
    LIN_enableDataReceiver(obj->linHandle);

    //
    // Set Baud Rate Settings - 160MHz Device
    //
	LIN_setMaximumBaudRate(obj->linHandle, 16000000U, 20000U);

	//
	// Set the Baud Rate
	//
	LIN_setBaudRatePrescaler(obj->linHandle, 9, 14);

	//
	// Set character length as 8-bits
	//
	LIN_setSCICharLength(obj->linHandle, 8);

	//
	// Set to 1 character in response field
	//
	LIN_setSCIFrameLength(obj->linHandle, 5);

    //
    // Enable the DMA to receive
    //
    LIN_enableSCIInterrupt(obj->linHandle, LIN_SCI_INT_RX);

    //
    // Set the interrupt priority to line 0 (high)
    //
    LIN_setSCIInterruptLevel0(obj->linHandle, LIN_SCI_INT_RX);

	// Exit LIN reset state
	//
	LIN_exitSoftwareReset(obj->linHandle);
} // end of HAL_setupLIN() function
