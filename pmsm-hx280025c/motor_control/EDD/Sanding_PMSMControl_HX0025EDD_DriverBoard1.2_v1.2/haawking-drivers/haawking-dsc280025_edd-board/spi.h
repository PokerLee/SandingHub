...
SPI_setcharLength(uint32_t base, uint16_t charLength)
{
    ...
    HWREG(base + SPI_O_CCR) = (HWREG(base + SPI_O_CCR) & ~SPI_CCR_SPICHAR_M) |
                               (charLength - 1U);
...
SPI_enableFIFO(uint32_t base)
{
    ...
    // Enable the FIFO.
    //
    HWREG(base + SPI_O_FFTX) |= SPI_FFTX_SPIFFENA | SPI_FFTX_TXFIFO;
    HWREG(base + SPI_O_FFRX) |= SPI_FFRX_RXFIFORESET;
}
...
SPI_resetTxFIFO(uint32_t base)
{
    ...
    // Reset the TX FIFO.
    //
    HWREG(base + SPI_O_FFTX) &= ~SPI_FFTX_TXFIFO;
    HWREG(base + SPI_O_FFTX) |= SPI_FFTX_TXFIFO;
}
                          ...
                          SPI_RxFIFOLevel rxLevel)
{
    ...
    //
    HWREG(base + SPI_O_FFTX) = (HWREG(base + SPI_O_FFTX) &
                                 (~SPI_FFTX_TXFFIL_M)) | (uint32_t)txLevel;
    HWREG(base + SPI_O_FFRX) = (HWREG(base + SPI_O_FFRX) &
                                 (~SPI_FFRX_RXFFIL_M)) | (uint32_t)rxLevel;
}
                          ...
                          SPI_RxFIFOLevel *rxLevel)
{
    ...
    //
    *txLevel = (SPI_TxFIFOLevel)(HWREG(base + SPI_O_FFTX) &
                                 SPI_FFTX_TXFFIL_M);
    *rxLevel = (SPI_RxFIFOLevel)(HWREG(base + SPI_O_FFRX) &
                                 SPI_FFRX_RXFFIL_M);
}
...
//
//*****************************************************************************
static __always_inline SPI_TxFIFOLevel
SPI_getTxFIFOStatus(uint32_t base)
{
    ...
    //
    return((SPI_TxFIFOLevel)((HWREG(base + SPI_O_FFTX) & SPI_FFTX_TXFFST_M) >>
                             SPI_FFTX_TXFFST_S));
}
...
//
//*****************************************************************************
static __always_inline SPI_RxFIFOLevel
SPI_getRxFIFOStatus(uint32_t base)
{
    ...
    // Get the current FIFO status
    //
    return((SPI_RxFIFOLevel)((HWREG(base + SPI_O_FFRX) & SPI_FFRX_RXFFST_M) >>
                             SPI_FFRX_RXFFST_S));
}
...
SPI_isBusy(uint32_t base)
{
    ...
    //
    return((HWREG(base + SPI_O_FFTX) & SPI_FFTX_TXFFST_M) != 0U);
}
...
//*****************************************************************************
static __always_inline void
SPI_writeDataNonBlocking(uint32_t base, uint32_t data)
{
    //
    // Check the arguments.
    //
...
//*****************************************************************************
static __always_inline void
SPI_writeDataBlockingFIFO(uint32_t base, uint32_t data)
{
    //
    // Check the arguments.
    //
...
//*****************************************************************************
static __always_inline void
SPI_writeDataBlockingNonFIFO(uint32_t base, uint32_t data)
{
    //
    // Check the arguments.
    //
    ...
    //
    while((HWREG(base + SPI_O_STS) & SPI_STS_BUFFULL_FLAG) != 0U)
    {
    }
...
SPI_readDataBlockingNonFIFO(uint32_t base)
{
    ...
    //
    while((HWREG(base + SPI_O_STS) & SPI_STS_INT_FLAG) == 0U)
    {
    }
...
SPI_enableTriWire(uint32_t base)
{
    ...
    //
    HWREG(base + SPI_O_PRI) |= SPI_PRI_TRIWIRE;
}
...
SPI_enableLoopback(uint32_t base)
{
    ...
    //
    HWREG(base + SPI_O_CCR) |= SPI_CCR_SPILBK;
}
...
SPI_setPTESignalPolarity(uint32_t base, SPI_PTEPolarity polarity)
{
    ...
    //
    HWREG(base + SPI_O_PRI) = (HWREG(base + SPI_O_PRI) & ~SPI_PRI_PTEINV) |
                               (uint32_t)polarity;
}
...
SPI_setEmulationMode(uint32_t base, SPI_EmulationMode mode)
{
    ...
    //
    HWREG(base + SPI_O_PRI) = (HWREG(base + SPI_O_PRI) &
...
//*****************************************************************************
static __always_inline void
SPI_setTxFifoTransmitDelay(uint32_t base, uint32_t delay)
{
    //
    // Check the arguments.
    //
...
SPI_enableTalk(uint32_t base)
{
    ...
    // Set the TALK bit
    //
    HWREG(base + SPI_O_CTL) |= SPI_CTL_TALK;
}
...
SPI_reset(uint32_t base)
{
    ...
    //
    HWREG(base + SPI_O_FFTX) &= ~SPI_FFTX_SPIRST;
    HWREG(base + SPI_O_FFTX) |= SPI_FFTX_SPIRST;
}
