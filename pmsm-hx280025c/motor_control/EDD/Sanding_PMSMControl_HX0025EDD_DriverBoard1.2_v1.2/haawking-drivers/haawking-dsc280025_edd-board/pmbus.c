...
//
const uint32_t PMBus_crc8Table[256U] = {
...
//
//*****************************************************************************
void PMBus_generateCRCTable(uint32_t *crcTable)
{
    register uint32_t i;
    register uint32_t j;
    register uint32_t crc8_accum;
    ...
    for(i = 0U; i < 256U; i++)
    {
        ...
        for(j = 0U; j < 8U; j++)
        {
            ...
            {
                crc8_accum = ((crc8_accum << 1U) & 0xffU) ^ (uint32_t)0x07U;
            }
            else
            {
...
//*****************************************************************************
bool
PMBus_verifyPEC(uint32_t base, uint32_t *buffer, const uint32_t *crcTable,
                uint32_t byteCount, uint32_t pec)
{
    register uint32_t i;
    register uint32_t parity = 0U;
    register uint32_t tableIndex;
    ...
    for(i = 0U; i < byteCount; i++)
    {
        ...
        tableIndex = accumulator ^ (uint32_t)__byte((int16_t *)buffer, parity);
        accumulator = crcTable[tableIndex];
        parity += 4U;
    }
...
PMBus_getData(uint32_t base, uint32_t *buffer, uint32_t status)
{
    ...
    //
    num_rx_bytes = (uint32_t)((status & PMBUS_PMBSTS_RD_BYTE_COUNT_M) >>
    ...
    if(num_rx_bytes != 0U)
    {
        ...
        {
            buffer[i] = (uint32_t)(temp & 0x000000FFUL);
            temp      = temp >> 8U;
        }
    }
