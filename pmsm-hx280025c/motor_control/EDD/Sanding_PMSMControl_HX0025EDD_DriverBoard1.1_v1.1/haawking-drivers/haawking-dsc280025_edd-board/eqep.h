...
EQEP_enableModule(uint32_t base)
{
    ...
    //
    HWREG(base + EQEP_O_QEPCTL) |= EQEP_QEPCTL_QPEN;
}
                              ...
                              uint32_t maxPosition)
{
    ...
    //
    HWREG(base + EQEP_O_QEPCTL) = (HWREG(base + EQEP_O_QEPCTL) &
...
//
//*****************************************************************************
static __always_inline int16_t
EQEP_getDirection(uint32_t base)
{
    ...
    //
    if((HWREG(base + EQEP_O_QEPSTS) & EQEP_QEPSTS_QDF) != 0U)
    {
        direction = 1;
    }
    else
...
EQEP_enableInterrupt(uint32_t base, uint16_t intFlags)
{
    ...
    //
    HWREG(base + EQEP_O_QEINT) |= intFlags;
}
...
EQEP_clearInterruptStatus(uint32_t base, uint16_t intFlags)
{
    ...
    //
    HWREG(base + EQEP_O_QCLR) = intFlags;
}
...
EQEP_forceInterrupt(uint32_t base, uint16_t intFlags)
{
    ...
    //
    HWREG(base + EQEP_O_QFRC) |= intFlags;
}
...
EQEP_getError(uint32_t base)
{
    ...
    //
    return((HWREG(base + EQEP_O_QFLG) & EQEP_QFLG_PHE) != 0U);
}
...
EQEP_getStatus(uint32_t base)
{
    ...
    //
    return(HWREG(base + EQEP_O_QEPSTS) & 0x00FFU);
}
...
EQEP_clearStatus(uint32_t base, uint16_t statusFlags)
{
    ...
    // Clear the requested interrupt sources.
    //
    HWREG(base + EQEP_O_QEPSTS) = statusFlags;
}
...
EQEP_enableCapture(uint32_t base)
{
    ...
    // Enable edge capture.
    //
    HWREG(base + EQEP_O_QCAPCTL) |= EQEP_QCAPCTL_CEN;
}
...
EQEP_getCapturePeriod(uint32_t base)
{
    ...
    //
    return(HWREG(base + EQEP_O_QCPRD));
}
...
EQEP_getCaptureTimer(uint32_t base)
{
    ...
    // Return the capture timer value.
    //
    return(HWREG(base + EQEP_O_QCTMR));
}
...
EQEP_enableCompare(uint32_t base)
{
    ...
    //
    HWREG(base + EQEP_O_QPOSCTL) |= EQEP_QPOSCTL_PCE;
}
...
EQEP_enableUnitTimer(uint32_t base, uint32_t period)
{
    ...
    //
    HWREG(base + EQEP_O_QEPCTL) |= EQEP_QEPCTL_UTE;
}
...
EQEP_enableWatchdog(uint32_t base, uint16_t period)
{
    ...
    //
    HWREG(base + EQEP_O_QWDPRD) = period;

    //
    // Enable peripheral watchdog.
    //
    HWREG(base + EQEP_O_QEPCTL) |= EQEP_QEPCTL_WDE;
}
...
EQEP_setWatchdogTimerValue(uint32_t base, uint16_t value)
{
    ...
    //
    HWREG(base + EQEP_O_QWDTMR) = value;
}
...
EQEP_getWatchdogTimerValue(uint32_t base)
{
    ...
    //
    return(HWREG(base + EQEP_O_QWDTMR));
}
...
EQEP_setSWPositionInit(uint32_t base, bool initialize)
{
    ...
    if(initialize)
    {
        HWREG(base + EQEP_O_QEPCTL) |= EQEP_QEPCTL_SWI;
    }
    else
    {
        HWREG(base + EQEP_O_QEPCTL) &= ~EQEP_QEPCTL_SWI;
...
EQEP_getCaptureTimerLatch(uint32_t base)
{
    ...
    // Return the current position counter.
    //
    return(HWREG(base + EQEP_O_QCTMRLAT));
}
...
EQEP_getCapturePeriodLatch(uint32_t base)
{
    ...
    // Return the current position counter.
    //
    return(HWREG(base + EQEP_O_QCPRDLAT));
}
...
EQEP_setQMAModuleMode(uint32_t base, EQEP_QMAMode qmaMode)
{
    ...
    //
    HWREG(base + EQEP_O_QMACTRL) =
        (HWREG(base + EQEP_O_QMACTRL) & ~EQEP_QMACTRL_MODE_M) |
        (uint16_t)qmaMode;
}
...
EQEP_setStrobeSource(uint32_t base, EQEP_StrobeSource strobeSrc)
{
    ...
    //
    HWREG(base + EQEP_O_QEPSTROBESEL) =
        (HWREG(base + EQEP_O_QEPSTROBESEL) & ~EQEP_QEPSTROBESEL_STROBESEL_M) |
        (uint16_t)strobeSrc;
}
...
EQEP_enableDirectionChangeDuringIndex(uint32_t base)
{
    ...
    //
    HWREG(base + EQEP_O_QDECCTL) |= EQEP_QDECCTL_QIDIRE;
}
...
EQEP_setEmulationMode(uint32_t base, EQEP_EmulationMode emuMode)
{
    ...
    //
    HWREG(base + EQEP_O_QEPCTL) =
        (HWREG(base + EQEP_O_QEPCTL) & ~EQEP_QEPCTL_FREE_SOFT_M) |
