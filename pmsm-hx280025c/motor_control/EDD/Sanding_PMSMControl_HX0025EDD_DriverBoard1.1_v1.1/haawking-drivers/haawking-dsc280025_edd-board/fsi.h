...
// FILE:   fsi.h
//
// TITLE:  H28x FSI Driver API header file
//
//###########################################################################
...
//!          13) recieved ping frame with matched tag
//!          14) recieved data frame with matched tag
//!          15) recieved error frame with tag match
//
//*****************************************************************************
...
#define FSI_RX_EVT_PING_FRAME_TAG_MATCH  (0x1000U)
#define FSI_RX_EVT_DATA_FRAME_TAG_MATCH  (0x2000U)
#define FSI_RX_EVT_ERR_FRAME_TAG_MATCH   (0x4000U)
...
FSI_sendTxFlush(uint32_t base)
{
    ...
    EALLOW;
    HWREG(base + FSI_O_TX_MAIN_CTRL) |= ((uint32_t)FSI_TX_MAIN_CTRL_FLUSH |
                                           (FSI_CTRL_REG_KEY <<
...
FSI_stopTxFlush(uint32_t base)
{
    ...
    EALLOW;
    HWREG(base + FSI_O_TX_MAIN_CTRL) = (((~FSI_TX_MAIN_CTRL_FLUSH) &
...
FSI_selectTxPLLClock(uint32_t base)
{
    ...
    //
    HWREG(base + FSI_O_TX_OPER_CTRL_LO) |= FSI_TX_OPER_CTRL_LO_SEL_PLLCLK;
...
FSI_enableTxClock(uint32_t base)
{
    ...
    // Enable Tx clock
    //
    HWREG(base + FSI_O_TX_CLK_CTRL) |= FSI_TX_CLK_CTRL_CLK_EN;
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
FSI_configPrescalar(uint32_t base, uint32_t preScaleValue)
{
    //
    // Check the arguments.
    //
    ...
    // Set prescalar value
    //
    HWREG(base + FSI_O_TX_CLK_CTRL) = (HWREG(base + FSI_O_TX_CLK_CTRL) &
...
FSI_setTxDataWidth(uint32_t base, FSI_DataWidth dataWidth)
{
    ...
    EALLOW;
    HWREG(base + FSI_O_TX_OPER_CTRL_LO) =
                                (HWREG(base + FSI_O_TX_OPER_CTRL_LO) &
...
FSI_enableTxSPIMode(uint32_t base)
{
    ...
    EALLOW;
    HWREG(base + FSI_O_TX_OPER_CTRL_LO) |= FSI_TX_OPER_CTRL_LO_SPI_MODE;
    EDIS;
}
...
FSI_setTxStartMode(uint32_t base, FSI_TxStartMode txStartMode)
{
    ...
    HWREG(base + FSI_O_TX_OPER_CTRL_LO) =
                    ...
                    ((uint32_t)txStartMode << FSI_TX_OPER_CTRL_LO_START_MODE_S);
    EDIS;
}
...
//*****************************************************************************
//
//! \brief Enables the Tx TDM mode for multi-slave configuration
//!
//! \param[in] base is the FSI Tx module base address
//!
//! \return None.
...
FSI_enableTxTDMMode(uint32_t base)
{
    ...
    // Enable Tx TDM Mode
    //
    HWREG(base + FSI_O_TX_OPER_CTRL_LO) |= FSI_TX_OPER_CTRL_LO_TDM_ENABLE;
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
FSI_setTxExtFrameTrigger(uint32_t base, uint32_t extInputNum)
{
    //
    // Check the arguments.
    //
    ...
    EALLOW;
    HWREG(base + FSI_O_TX_OPER_CTRL_HI) =
                          (HWREG(base + FSI_O_TX_OPER_CTRL_HI) &
...
FSI_enableTxCRCForceError(uint32_t base)
{
    ...
    EALLOW;
    HWREG(base + FSI_O_TX_OPER_CTRL_HI) |= FSI_TX_OPER_CTRL_HI_FORCE_ERR;
    EDIS;
}
...
FSI_setTxECCComputeWidth(uint32_t base, FSI_ECCComputeWidth eccComputeWidth)
{
    ...
    {
        HWREG(base + FSI_O_TX_OPER_CTRL_HI) |= FSI_TX_OPER_CTRL_HI_ECC_SEL;
    }
    else
    {
        HWREG(base + FSI_O_TX_OPER_CTRL_HI) &= ~FSI_TX_OPER_CTRL_HI_ECC_SEL;
...
FSI_setTxFrameType(uint32_t base, FSI_FrameType frameType)
{
    ...
    HWREG(base + FSI_O_TX_FRAME_CTRL) = (HWREG(base + FSI_O_TX_FRAME_CTRL) &
...
//*****************************************************************************
static __always_inline void
FSI_setTxSoftwareFrameSize(uint32_t base, uint32_t nWords)
{
    //
    // Check the arguments.
    //
...
FSI_startTxTransmit(uint32_t base)
{
    ...
    HWREG(base + FSI_O_TX_FRAME_CTRL) |= FSI_TX_FRAME_CTRL_START;
}
...
FSI_setTxFrameTag(uint32_t base, FSI_FrameTag frameTag)
{
    ...
    HWREG(base + FSI_O_TX_FRAME_TAG_UDATA) =
                            (HWREG(base + FSI_O_TX_FRAME_TAG_UDATA) &
...
//*****************************************************************************
static __always_inline void
FSI_setTxUserDefinedData(uint32_t base, uint32_t userDefData)
{
    //
    // Check the arguments.
    //
...
//*****************************************************************************
static __always_inline void
FSI_setTxBufferPtr(uint32_t base, uint32_t bufPtrOff)
{
    //
    // Check the arguments.
    //
    ...
    EALLOW;
    HWREG(base + FSI_O_TX_BUF_PTR_LOAD) = bufPtrOff;
    EDIS;
}
...
FSI_getTxBufferPtr(uint32_t base)
{
    ...
    return(HWREG(base + FSI_O_TX_BUF_PTR_STS) &
                ...
                FSI_FrameTag pingFrameTag)
{
    ...
    HWREG(base + FSI_O_TX_PING_TAG) = (uint32_t)pingFrameTag;

    EALLOW;

    HWREG(base + FSI_O_TX_PING_TO_REF) = refValue;
    HWREG(base + FSI_O_TX_PING_CTRL) |= FSI_TX_PING_CTRL_TIMER_EN;
...
FSI_setTxPingTag(uint32_t base, FSI_FrameTag frameTag)
{
    ...
    HWREG(base + FSI_O_TX_PING_TAG) = (uint32_t)frameTag;
}
...
//*****************************************************************************
static __always_inline void
FSI_enableTxExtPingTrigger(uint32_t base, uint32_t extTrigSel)
{
    //
    // Check the arguments.
    //
    ...
    //
    HWREG(base + FSI_O_TX_PING_CTRL) = (HWREG(base + FSI_O_TX_PING_CTRL) &
    ...
    //
    HWREG(base + FSI_O_TX_PING_CTRL) |= FSI_TX_PING_CTRL_EXT_TRIG_EN;
    EDIS;
}
...
FSI_enableTxDMAEvent(uint32_t base)
{
    ...
    EALLOW;
    HWREG(base + FSI_O_TX_DMA_CTRL) |= FSI_TX_DMA_CTRL_DMA_EVT_EN;
    EDIS;
}
...
FSI_lockTxCtrl(uint32_t base)
{
    ...
    EALLOW;
    HWREG(base + FSI_O_TX_LOCK_CTRL) = ((uint32_t)FSI_TX_LOCK_CTRL_LOCK |
                                        (FSI_CTRL_REG_KEY <<
...
FSI_getTxEventStatus(uint32_t base)
{
    ...
    return(HWREG(base + FSI_O_TX_EVT_STS) & FSI_TX_EVTMASK);
}
...
//*****************************************************************************
static __always_inline void
FSI_forceTxEvents(uint32_t base, uint32_t evtFlags)
{
    //
    // Check the arguments.
    //
    ...
    EALLOW;
    HWREG(base + FSI_O_TX_EVT_FRC) = (evtFlags & FSI_TX_EVTMASK);
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
FSI_clearTxEvents(uint32_t base, uint32_t evtFlags)
{
    //
    // Check the arguments.
    //
    ...
    EALLOW;
    HWREG(base + FSI_O_TX_EVT_CLR) = (evtFlags & FSI_TX_EVTMASK);
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
FSI_enableTxUserCRC(uint32_t base, uint32_t userCRCValue)
{
    //
    // Check the arguments.
    //
    ...
    EALLOW;
    HWREG(base + FSI_O_TX_OPER_CTRL_LO) |= FSI_TX_OPER_CTRL_LO_SW_CRC;
    EDIS;

    HWREG(base + FSI_O_TX_USER_CRC) = userCRCValue;
}
...
FSI_getTxECCValue(uint32_t base)
{
    ...
    return(HWREG(base + FSI_O_TX_ECC_VAL) & FSI_TX_ECC_VAL_ECC_VAL_M);
}
                      ...
                      uint32_t intFlags)
{
    ...
    if(intNum == FSI_INT1)
    {
        HWREG(base + FSI_O_TX_INT_CTRL) |= (intFlags & FSI_TX_EVTMASK);
    }
    else
    {
        HWREG(base + FSI_O_TX_INT_CTRL) |= ((intFlags & FSI_TX_EVTMASK) <<
...
FSI_enableRxInternalLoopback(uint32_t base)
{
    ...
    HWREG(base + FSI_O_RX_MAIN_CTRL) |=
                                ((uint32_t)FSI_RX_MAIN_CTRL_INT_LOOPBACK |
...
FSI_enableRxSPIPairing(uint32_t base)
{
    ...
    EALLOW;
    HWREG(base + FSI_O_RX_MAIN_CTRL) |=
                            ((uint32_t)FSI_RX_MAIN_CTRL_SPI_PAIRING |
                             (FSI_CTRL_REG_KEY << FSI_RX_MAIN_CTRL_KEY_S));
    EDIS;
}
...
FSI_disableRxSPIPairing(uint32_t base)
{
    ...
    EALLOW;
    HWREG(base + FSI_O_RX_MAIN_CTRL) = (HWREG(base + FSI_O_RX_MAIN_CTRL) &
...
FSI_setRxDataWidth(uint32_t base, FSI_DataWidth dataWidth)
{
    ...
    EALLOW;
    HWREG(base + FSI_O_RX_OPER_CTRL) = (HWREG(base + FSI_O_RX_OPER_CTRL) &
...
FSI_enableRxSPIMode(uint32_t base)
{
    ...
    EALLOW;
    HWREG(base + FSI_O_RX_OPER_CTRL) |= FSI_RX_OPER_CTRL_SPI_MODE;
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
FSI_setRxSoftwareFrameSize(uint32_t base, uint32_t nWords)
{
    //
    // Check the arguments.
    //
...
FSI_setRxECCComputeWidth(uint32_t base, FSI_ECCComputeWidth eccComputeWidth)
{
    ...
    if(eccComputeWidth == FSI_16BIT_ECC_COMPUTE)
    {
        HWREG(base + FSI_O_RX_OPER_CTRL) |= FSI_RX_OPER_CTRL_ECC_SEL;
    }
    else
    {
        HWREG(base + FSI_O_RX_OPER_CTRL) &= ~FSI_RX_OPER_CTRL_ECC_SEL;
...
FSI_setRxPingTimeoutMode(uint32_t base, FSI_PingTimeoutMode pingTimeoutMode)
{
    ...
    if(pingTimeoutMode == FSI_PINGTIMEOUT_ON_HWSWINIT_PING_FRAME)
    {
        HWREG(base + FSI_O_RX_OPER_CTRL) |= FSI_RX_OPER_CTRL_PING_WD_RST_MODE;
    }
    else
    {
        HWREG(base + FSI_O_RX_OPER_CTRL) &= ~FSI_RX_OPER_CTRL_PING_WD_RST_MODE;
...
//
//*****************************************************************************
static __always_inline FSI_FrameType
FSI_getRxFrameType(uint32_t base)
{
    ...
    return((FSI_FrameType)(HWREG(base + FSI_O_RX_FRAME_INFO) &
...
FSI_enableRxDMAEvent(uint32_t base)
{
    ...
    EALLOW;
    HWREG(base + FSI_O_RX_DMA_CTRL) |= FSI_RX_DMA_CTRL_DMA_EVT_EN;
    EDIS;
}
...
FSI_getRxFrameTag(uint32_t base)
{
    ...
    return((HWREG(base + FSI_O_RX_FRAME_TAG_UDATA) &
...
FSI_getRxEventStatus(uint32_t base)
{
    ...
    return(HWREG(base + FSI_O_RX_EVT_STS) & FSI_RX_EVTMASK);
}
...
//*****************************************************************************
static __always_inline void
FSI_forceRxEvents(uint32_t base, uint32_t evtFlags)
{
    //
    // Check the arguments.
    //
    ...
    EALLOW;
    HWREG(base + FSI_O_RX_EVT_FRC) = (evtFlags & FSI_RX_EVTMASK);
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
FSI_clearRxEvents(uint32_t base, uint32_t evtFlags)
{
    //
    // Check the arguments.
    //
    ...
    EALLOW;
    HWREG(base + FSI_O_RX_EVT_CLR) = (evtFlags & FSI_RX_EVTMASK);
    EDIS;
}
...
FSI_getRxReceivedCRC(uint32_t base)
{
    ...
    return(HWREG(base + FSI_O_RX_CRC_INFO) & FSI_RX_CRC_INFO_RX_CRC_M);
}
...
FSI_getRxComputedCRC(uint32_t base)
{
    ...
    return((HWREG(base + FSI_O_RX_CRC_INFO) &
...
//*****************************************************************************
static __always_inline void
FSI_setRxBufferPtr(uint32_t base, uint32_t bufPtrOff)
{
    //
    // Check the arguments.
    //
    ...
    EALLOW;
    HWREG(base + FSI_O_RX_BUF_PTR_LOAD) = bufPtrOff;
    EDIS;
}
...
FSI_getRxBufferPtr(uint32_t base)
{
    ...
    return(HWREG(base + FSI_O_RX_BUF_PTR_STS) &
...
FSI_enableRxFrameWatchdog(uint32_t base, uint32_t wdRef)
{
    ...
    HWREG(base + FSI_O_RX_FRAME_WD_CTRL) |= FSI_RX_FRAME_WD_CTRL_FRAME_WD_EN;
    EDIS;
}
...
FSI_enableRxPingWatchdog(uint32_t base, uint32_t wdRef)
{
    ...
    HWREG(base + FSI_O_RX_PING_WD_CTRL) |= FSI_RX_PING_WD_CTRL_PING_WD_EN;
    EDIS;
}
...
FSI_getRxPingTag(uint32_t base)
{
    ...
    return((HWREG(base + FSI_O_RX_PING_TAG) & FSI_RX_PING_TAG_PING_TAG_M) >>
...
FSI_lockRxCtrl(uint32_t base)
{
    ...
    EALLOW;
    HWREG(base + FSI_O_RX_LOCK_CTRL) = ((uint32_t)FSI_RX_LOCK_CTRL_LOCK |
                                        (FSI_CTRL_REG_KEY <<
...
//*****************************************************************************
static __always_inline void
FSI_setRxReceivedECCValue(uint32_t base, uint32_t rxECCvalue)
{
    //
    // Check the arguments.
    //
    ...
    HWREG(base + FSI_O_RX_ECC_VAL) = rxECCvalue;
}
...
FSI_getRxECCLog(uint32_t base)
{
    ...
    return(HWREG(base + FSI_O_RX_ECC_LOG) &
                      ...
                      uint32_t intFlags)
{
    ...
    if(intNum == FSI_INT1)
    {
        HWREG(base + FSI_O_RX_INT1_CTRL) |= (intFlags &
                                             FSI_RX_EVTMASK);
    }
    else
    {
        HWREG(base + FSI_O_RX_INT2_CTRL) |= (intFlags &
                                             FSI_RX_EVTMASK);
    }
...
//*****************************************************************************
static __always_inline void
FSI_setRxFrameTagRef(uint32_t base, uint32_t refVal)
{
    //
    // Check the arguments.
    //
    ...
    //
    HWREG(base + FSI_O_RX_FRAME_TAG_CMP) =
          ((HWREG(base + FSI_O_RX_FRAME_TAG_CMP) &
           ~((uint32_t)FSI_RX_FRAME_TAG_CMP_TAG_REF_M)) |
...
FSI_getRxFrameTagRef(uint32_t base)
{
    ...
    //
    return((HWREG(base + FSI_O_RX_FRAME_TAG_CMP) &
...
//*****************************************************************************
static __always_inline void
FSI_setRxFrameTagMask(uint32_t base, uint32_t maskVal)
{
    //
    // Check the arguments.
    //
    ...
    HWREG(base + FSI_O_RX_FRAME_TAG_CMP) =
          ((HWREG(base + FSI_O_RX_FRAME_TAG_CMP) &
           ~((uint32_t)FSI_RX_FRAME_TAG_CMP_TAG_MASK_M)) |
...
FSI_enableRxFrameTagCompare(uint32_t base)
{
    ...
    //
    HWREG(base + FSI_O_RX_FRAME_TAG_CMP) |= FSI_RX_FRAME_TAG_CMP_CMP_EN;
    EDIS;
}
...
FSI_disableRxFrameTagCompare(uint32_t base)
{
    ...
    //
    HWREG(base + FSI_O_RX_FRAME_TAG_CMP) &=
                                   ~(uint32_t)FSI_RX_FRAME_TAG_CMP_CMP_EN;
    EDIS;
}
...
FSI_enableRxFrameBroadcast(uint32_t base)
{
    ...
    //
    HWREG(base + FSI_O_RX_FRAME_TAG_CMP) |= FSI_RX_FRAME_TAG_CMP_BROADCAST_EN;
    EDIS;
}
...
FSI_disableRxFrameBroadcast(uint32_t base)
{
    ...
    //
    HWREG(base + FSI_O_RX_FRAME_TAG_CMP) &=
                                  ~(uint32_t)FSI_RX_FRAME_TAG_CMP_BROADCAST_EN;
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
FSI_setRxPingTagRef(uint32_t base, uint32_t refVal)
{
    //
    // Check the arguments.
    //
    ...
    //
    HWREG(base + FSI_O_RX_PING_TAG_CMP) =
          ((HWREG(base + FSI_O_RX_PING_TAG_CMP) &
           ~((uint32_t)FSI_RX_PING_TAG_CMP_TAG_REF_M)) |
...
FSI_getRxPingTagRef(uint32_t base)
{
    ...
    //
    return((HWREG(base + FSI_O_RX_PING_TAG_CMP) &
...
//*****************************************************************************
static __always_inline void
FSI_setRxPingTagMask(uint32_t base, uint32_t maskVal)
{
    //
    // Check the arguments.
    //
    ...
    HWREG(base + FSI_O_RX_PING_TAG_CMP) =
          ((HWREG(base + FSI_O_RX_PING_TAG_CMP) &
           ~((uint32_t)FSI_RX_PING_TAG_CMP_TAG_MASK_M)) |
...
FSI_enableRxPingTagCompare(uint32_t base)
{
    ...
    //
    HWREG(base + FSI_O_RX_PING_TAG_CMP) |= FSI_RX_PING_TAG_CMP_CMP_EN;
    EDIS;
}
...
FSI_disableRxPingTagCompare(uint32_t base)
{
    ...
    //
    HWREG(base + FSI_O_RX_PING_TAG_CMP) &=
                                   ~(uint32_t)FSI_RX_PING_TAG_CMP_CMP_EN;
    EDIS;
}
...
FSI_enableRxPingBroadcast(uint32_t base)
{
    ...
    //
    HWREG(base + FSI_O_RX_PING_TAG_CMP) |= FSI_RX_PING_TAG_CMP_BROADCAST_EN;
    EDIS;
}
...
FSI_disableRxPingBroadcast(uint32_t base)
{
    ...
    //
    HWREG(base + FSI_O_RX_PING_TAG_CMP) &=
                                  ~(uint32_t)FSI_RX_PING_TAG_CMP_BROADCAST_EN;
    EDIS;
}
//*****************************************************************************
//
...
//*****************************************************************************
extern void
FSI_performTxInitialization(uint32_t base, uint32_t prescalar);
...
//*****************************************************************************
extern void
FSI_executeTxFlushSequence(uint32_t base, uint32_t prescalar);
