...
typedef struct _MOTOR_SetVars_t_
{
    ...
    float32_t KpSuTrim_pos;
    float32_t KiSuTrim_pos;

    float32_t KpSuTrim_spd;
    float32_t KiSuTrim_spd;

    float32_t KpSuTrim_Id;
    float32_t KiSuTrim_Id;

    float32_t KpSuTrim_Iq;
    float32_t KiSuTrim_Iq;
...
//    bool flagUpdateMTPAParams;
    ...
    UENC_HWState_e uENCState;	// 手轮编码器输入
    ...
    bool flagPosSwitch;						//!<
    bool flagPosTrimStable;					//!<
    bool flagPosSTrimStable;				//!<

    uint32_t  posCount;
    uint32_t  posTrimStableCount;
    uint32_t  posSTrimStableCount;
//    uint32_t  posDeltaCount;
    ...
    float32_t posFastPrev_rad;				//!< Current mechanical Angle position information, rad
    float32_t posLowPrev_rad;				//!< Current mechanical Angle position information, rad
    float32_t posFastDelta_rad;				//!< 快速路径下的弧度变化, rad
    float32_t posLowDelta_rad;				//!< 低速路径下的弧度变化, rad
    float32_t posLowDeltaAccum_rad;			//!< 低速路径下的弧度累计值, rad
    float32_t posLowDeltaThreshold_rad;
    float32_t posLowTimeAccum_s;
    float32_t posLowTimeThreshold_rad;
...
#if MTR1_ABS_ENC
    uint32_t  encFastSpdCnt;				//!<
    uint32_t  encFastSpdThreshold;			//!<
    uint32_t  encLowSpdCnt;					//!<
    uint32_t  encLowSpdThreshold;			//!<
    float32_t encLowSpdCompe;
    float32_t encLowSpdCompeUI;
#endif
    ...
    float32_t speedPrev_Hz;
    float32_t speed_Rpm;					//!< Current speed, rpm
    float32_t speedENC_Hz;					//!< the speed from Encoder module
    float32_t speedENCFast_Hz;					//!< the speed from Encoder module
    float32_t speedENCSlow_Hz;					//!< the speed from Encoder module
...
//	float32_t angleDelta_rad;				//!< angleDelta_rad多次控制周期下的累加和
...
//! \brief  Encoder handwheel speed detection
extern void encoderHandwheelSpeedDetection(MOTOR_Handle handle);
...
static inline void updateControllers(MOTOR_Handle handle)
{
    ...
    if(obj->mctrlState >= MCTRL_FIRST_RUN)
    {
	   if(handle->flagEnableShowMode)
	   {
	        ...
	        PI_setMinMax(obj->piHandle_spd,-18,18);

	        // update the position controller
	        PI_setGains(obj->piHandle_pos, objSets->Kp_pos, objSets->Ki_pos);

	        PI_setMinMax(obj->piHandle_pos,-15,15);
...
//		        obj->angleDeltaSumCount = 50;
		   ...
		   else if(obj->uCtrlState == UCTRL_SUPER_TRIM)
		   {
		        // update the Id controller
		        PI_setGains(obj->piHandle_Id,objSets->KpSuTrim_Id , objSets->KiSuTrim_Id);//objSets->Kp_Id

		        // update the Iq controller
		        PI_setGains(obj->piHandle_Iq, objSets->KpSuTrim_Iq, objSets->KiSuTrim_Iq);

		        // update the speed controller
		        PI_setGains(obj->piHandle_spd, objSets->KpSuTrim_spd, objSets->KiSuTrim_spd);

		        // update the position controller
		        PI_setGains(obj->piHandle_pos, objSets->KpSuTrim_pos, objSets->KiSuTrim_pos);

		        PI_setMinMax(obj->piHandle_pos,-50,50);

//		        obj->angleDeltaSumCount = 300;
		   }
		   else if(obj->uCtrlState == UCTRL_COARSE)
		   {
		        // update the Id controller
		        PI_setGains(obj->piHandle_Id,objSets->Kp_Id , objSets->Ki_Id);//objSets->Kp_Id
		        PI_setMinMax(obj->piHandle_Id,-3,3);

		        // update the Iq controller
		        PI_setGains(obj->piHandle_Iq, objSets->Kp_Iq, objSets->Ki_Iq);
		        PI_setMinMax(obj->piHandle_Iq,-3,3);

		        // update the speed controller
		        PI_setGains(obj->piHandle_spd, objSets->Kp_spd, objSets->Ki_spd);
		        PI_setMinMax(obj->piHandle_spd,-12,12);

		        // update the position controller
		        PI_setGains(obj->piHandle_pos, objSets->Kp_pos, objSets->Ki_pos);
		        PI_setMinMax(obj->piHandle_pos,-1.0,1.0);

//		        obj->angleDeltaSumCount = 500;
		   }
	   }
    }
}
