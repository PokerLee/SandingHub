...
void resetMotorControl(MOTOR_Handle handle)
{
    ...
    // disable the PWM
    if(handle->brakingMode != DAMPING_BRAKE_MODE)
    {
...
//    PI_setGains(obj->piHandle_Id, Kp_Id, Ki_Id);
    // GL35 0.01, 0.001
    // GL40 1,0.1
    PI_setGains(obj->piHandle_Id, 5.0,0.005);
    PI_setUi(obj->piHandle_Id, 0.0f);
    PI_setRefValue(obj->piHandle_Id, 0.0f);
...
//    PI_setGains(obj->piHandle_Iq, Kp_Iq, Ki_Iq);
    PI_setGains(obj->piHandle_Iq, 5.0,0.005);
    PI_setUi(obj->piHandle_Iq, 0.0f);
    PI_setRefValue(obj->piHandle_Iq, 0.0f);
    ...
    PI_setGains(obj->piHandle_spd, 2.0, 1.0E-3);
    PI_setUi(obj->piHandle_spd, 0.0f);
    PI_setRefValue(obj->piHandle_spd, 0.0f);
...
//    PI_setGains(obj->piHandle_pos, 4.5f, 0.0001f);
    PI_setGains(obj->piHandle_pos, 13.0f, 0.0f);
    PI_setUi(obj->piHandle_pos, 0.0f);
    PI_setRefValue(obj->piHandle_pos, 0.0f);
    PI_setFbackValue(obj->piHandle_pos, 0.0f);
    PI_setFfwdValue(obj->piHandle_pos, 0.0f);
    PI_setMinMax(obj->piHandle_pos, -6.0f, 6.0f);
    ...
    objSets->KpTrim_Id = 0.15f;
    objSets->KiTrim_Id = 5.0E-4f;

    // Get the Iq controller parameters
    objSets->KpTrim_Iq = 0.15f;
    objSets->KiTrim_Iq = 5.0E-4f;

    // Get the speed controller parameters
    objSets->KpTrim_spd = 2.0f;
    ...
    // Get the Id controller parameters
    objSets->KpSuTrim_Id = 0.3f;
    objSets->KiSuTrim_Id = 0.05f;

    // Get the Iq controller parameters
    objSets->KpSuTrim_Iq = 0.3f;
    objSets->KiSuTrim_Iq = 0.05f;

    // Get the speed controller parameters
    objSets->KpSuTrim_spd = 10.0f;
...
int eqepHWInput = 0;
int eqepHWInputLast = 0;
int eqepHWInputPrev = 0;
...
void posCheckMoinitor(MOTOR_Handle handle)
{
	...
	eqepHWInput = EQEP_getPosition(MTR1_EQEP_ENCODER_BASE);
	float32_t tempEqepCountDelta = eqepHWInput-eqepHWInputLast;
...
//			if(fabsf(obj->speedRef_Hz)<0.001)
			if(obj->uENCState == UENC_SLOW)
			{
				if(obj->flagPosSTrimStable == 1)
				{
			...
			{
//				if((obj->flagPosSTrimStable != 1)&&(obj->flagPosTrimStable == 1))
				if(obj->flagPosSTrimStable != 1)
				{
					obj->uCtrlState = UCTRL_TRIM;
				}
...
//				if((obj->flagPosSTrimStable != 1)&&(obj->flagPosTrimStable == 1))
	...
	{
		if(obj->uCtrlState != UCTRL_COARSE)
		{
			if(obj->flagPosSTrimStable == 1)
			{
				obj->uCtrlState = UCTRL_SUPER_TRIM;
	...
	if(obj->posFastDelta_rad>0.08726646)//5 deg
	{
		obj->uCtrlState = UCTRL_COARSE;
	}

	if(eqepTrimMonitor>=50000)
	{
		eqepTrimMonitor = 0;
		obj->flagPosSwitch = 0;
	}
...
float32_t eqepHWInput_Delta = 0.0f;
uint32_t eqepHWInput_Count = 0.0f;
void encoderHandwheelSpeedDetection(MOTOR_Handle handle)
{
	MOTOR_Vars_t *obj = (MOTOR_Vars_t *)handle;
	...
	if(eqepHWInput_Count>=200)//0.2s
	{
		...
		if(eqepHWInput_Delta!=0)
		{
			if(obj->direction == 1)
			{
				if(fabsf(eqepHWInput_Delta)<5)
				{
					obj->uENCState = UENC_SLOW;
				}
				else
				{
					obj->uENCState = UENC_FAST;
				}
			}
		...
		eqepHWInput_Delta = eqepHWInput-eqepHWInputPrev;
		eqepHWInputPrev = eqepHWInput;
	}
	eqepHWInput_Count++;
}
...
void stopMotorControl(MOTOR_Handle handle)
{
    ...
    if(obj->motorState == MOTOR_BRAKE_STOP)
    {
        ...
        {
			obj->flagEnableBraking = true; // one shot
			obj->motorState = MOTOR_BRAKE_STOP;
			obj->mctrlState = MCTRL_BRAKE_STOP;
        }
        else if(obj->brakingMode == HARDSWITCH_BRAKE_MODE)
        {
			// Exit braking PWM mode
			HAL_exitBrakeResetPWM(obj->halMtrHandle);

			obj->flagEnableBraking = false; // one shot
			obj->motorState = MOTOR_STOP_IDLE;
			obj->mctrlState = MCTRL_BRAKE_STOP;
        }
