...
// PLLSYSCLK = 10MHz (SYSCTL_OSCSRC_OSC2) * 12 (IMULT) / (1 (REFDIV) * 1 (ODIV) * 1(SYSDIV))
//

#define DEVICE_SETCLOCK_CFG          (SYSCTL_OSCSRC_OSC2| SYSCTL_IMULT(12) | \
...
//
#define DEVICE_SYSCLK_FREQ          ((DEVICE_OSCSRC_FREQ * 12) / (1 * 1* 1))
...
//
// Define to pass to SysCtl_setClock(). Will configure the clock as follows:
// PLLSYSCLK = 20MHz (SYSCTL_OSCSRC_OSC2) * 24 (IMULT) / (2 (REFDIV) * 2 (ODIV) * 1(SYSDIV))
//

#define DEVICE_SETCLOCK_CFG          (SYSCTL_OSCSRC_XTAL| SYSCTL_IMULT(24) | \
                                      SYSCTL_REFDIV(2) | SYSCTL_ODIV(2) |SYSCTL_VCO(4) |\
                                      SYSCTL_SYSDIV(1) | SYSCTL_PLL_ENABLE | \
                                      SYSCTL_DCC_BASE_0)
...
// code below if a different clock configuration is used!
//
#define DEVICE_SYSCLK_FREQ          ((DEVICE_OSCSRC_FREQ * 24) / (2 * 2* 1))
#elif defined(CPU_FREQ_160MHZ)
//
// Define to pass to SysCtl_setClock(). Will configure the clock as follows:
...
#define FSI_PRESCALE_20MHZ          3U
#define FSI_PRESCALE_15MHZ          4U
#define FSI_PRESCALE_10MHZ          6U
#define FSI_PRESCALE_7P5MHZ         8U
#elif defined(CPU_FREQ_160MHZ)
