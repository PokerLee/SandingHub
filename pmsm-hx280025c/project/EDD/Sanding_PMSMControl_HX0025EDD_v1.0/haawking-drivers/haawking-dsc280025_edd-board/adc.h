...
#define ADC_EXT_REF_TSSLOPE 0.006f
#define ADC_EXT_REF_TSOFFSET 1.1f
                           ...
                           ADC_IntSOCTrigger trigger)
{
    ...
    //
    shiftVal = (uint32_t)(socNumber % 8) << 1U;
    regOffset = (socNumber >> 3) * 4;
    ...
    //
    EALLOW;
    HWREG(base + ADC_O_INTSOCSEL1 + regOffset) = (HWREG(base + ADC_O_INTSOCSEL1 + regOffset) &
...
ADC_setInterruptPulseMode(uint32_t base, ADC_PulseMode pulseMode)
{
    ...
    HWREG(base + ADC_O_CTL1) = (HWREG(base + ADC_O_CTL1) &
                                 ~ADC_CTL1_INTPULSEPOS) | (uint32_t)pulseMode;
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
ADC_setInterruptCycleOffset(uint32_t base, uint32_t cycleOffset)
{
    //
    // Check the arguments.
    //
...
ADC_forceSOC(uint32_t base, ADC_SOCNumber socNumber)
{
    ...
    //
    HWREG(base + ADC_O_SOCFRC1) = (1U << (uint32_t)socNumber);
}
...
ADC_getInterruptStatus(uint32_t base, ADC_IntNumber adcIntNum)
{
    ...
    //
    return((HWREG(base + ADC_O_INTFLG) & (1U << (uint32_t)adcIntNum)) != 0U);
}
...
ADC_clearInterruptStatus(uint32_t base, ADC_IntNumber adcIntNum)
{
    ...
    //
    HWREG(base + ADC_O_INTFLGCLR) = 1U << (uint32_t)adcIntNum;
}
...
ADC_getInterruptOverflowStatus(uint32_t base, ADC_IntNumber adcIntNum)
{
    ...
    // Get the specified ADC interrupt status.
    //
    return((HWREG(base + ADC_O_INTOVF) & (1U << (uint32_t)adcIntNum)) != 0U);
}
...
ADC_clearInterruptOverflowStatus(uint32_t base, ADC_IntNumber adcIntNum)
{
    ...
    //
    HWREG(base + ADC_O_INTOVFCLR) = 1U << (uint32_t)adcIntNum;
}
...
ADC_readResult(uint32_t resultBase, ADC_SOCNumber socNumber)
{
    ...
    //
    return(HWREG(resultBase + ADC_RESULTx_OFFSET_BASE + socNumber*4));
}
...
//*****************************************************************************
static __always_inline void
ADC_setBurstModeConfig(uint32_t base, ADC_Trigger trigger, uint32_t burstSize)
{
    uint32_t regValue;
    ...
    regValue = (uint32_t)trigger | ((burstSize - 1U) <<
                                    ADC_BURSTCTL_BURSTSIZE_S);

    HWREG(base + ADC_O_BURSTCTL) = (HWREG(base + ADC_O_BURSTCTL) &
                                     ~((uint32_t)ADC_BURSTCTL_BURSTTRIGSEL_M |
...
ADC_enableBurstMode(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + ADC_O_BURSTCTL) |= ADC_BURSTCTL_BURSTEN;
    EDIS;
}
...
ADC_configOSDetectMode(uint32_t base, ADC_OSDetectMode modeVal)
{
    ...
    //
    EALLOW;
    HWREG(base + ADC_O_OSDETECT) = ((HWREG(base + ADC_O_OSDETECT) &
...
//*****************************************************************************
static __always_inline void
ADC_enablePPBEvent(uint32_t base, ADC_PPBNumber ppbNumber, uint32_t evtFlags)
{
    //
    // Check the arguments.
    //
    ...
    //
    EALLOW;
    HWREG(base + ADC_O_EVTSEL) |= evtFlags << ((uint32_t)ppbNumber * 4U);
    EDIS;
}
...
//*****************************************************************************
static __always_inline void
ADC_disablePPBEvent(uint32_t base, ADC_PPBNumber ppbNumber, uint32_t evtFlags)
{
    //
    // Check the arguments.
    //
                            ...
                            uint32_t intFlags)
{
    ...
    //
    EALLOW;
    HWREG(base + ADC_O_EVTINTSEL) |= intFlags << ((uint32_t)ppbNumber * 4U);
    EDIS;
}
                        ...
                        uint32_t evtFlags)
{
    ...
    //
    HWREG(base + ADC_O_EVTCLR) |= evtFlags << ((uint32_t)ppbNumber * 4U);
}
...
ADC_getPPBDelayTimeStamp(uint32_t base, ADC_PPBNumber ppbNumber)
{
    ...
    //
    return(HWREG(base + ppbOffset) & ADC_PPB2STAMP_DLYSTAMP_M);
}
...
ADC_enableInterrupt(uint32_t base, ADC_IntNumber adcIntNum)
{
    ...
    //
    intRegAddr = base + ADC_INTSELxNy_OFFSET_BASE + ((uint32_t)adcIntNum >> 1)*4;
    shiftVal = ((uint32_t)adcIntNum & 0x1U) << 3U;
    ...
    HWREG(intRegAddr) |= ADC_INTSEL1N2_INT1E << shiftVal;
                       ...
                       ADC_SOCNumber socNumber)
{
    ...
    HWREG(intRegAddr) =
        (HWREG(intRegAddr) & ~(ADC_INTSEL1N2_INT1SEL_M << shiftVal)) |
        ((uint32_t)socNumber << shiftVal);
...
ADC_enableContinuousMode(uint32_t base, ADC_IntNumber adcIntNum)
{
    ...
    HWREG(intRegAddr) |= ADC_INTSEL1N2_INT1CONT << shiftVal;
...
//*****************************************************************************
static __always_inline int32_t
ADC_getTemperatureC(uint32_t tempResult, float32_t vref)
{
    float32_t temp;
    ...
    //
    temp = (float32_t)(tempResult * vref / 4095.0f);
	return((int32_t)((temp - ADC_EXT_REF_TSOFFSET) /
					 ADC_EXT_REF_TSSLOPE));
}
...
//*****************************************************************************
static __always_inline int32_t
ADC_getTemperatureK(uint32_t tempResult, float32_t vref)
{
    float32_t temp;
