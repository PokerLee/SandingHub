...
//*****************************************************************************
//
//! The following are defines for the \e pin parameter of the
//! LIN_getPinStatus() function.
//
//*****************************************************************************
typedef enum
{
    LIN_PINTYPE_TX = 0x4U,    //!< Transmit Pin
    LIN_PINTYPE_RX = 0x2U     //!< Receive Pin
} LIN_PinType;
...
LIN_setLINMode(uint32_t base, LIN_LINMode mode)
{
    ...
    HWREG(base + LIN_O_SCIGCR1) = (HWREG(base + LIN_O_SCIGCR1) &
                                   ~LIN_SCIGCR1_CLK_COMMANDER) | (uint32_t)mode;
...
LIN_setMaximumBaudRate(uint32_t base, uint32_t systemClock)
{
    ...
    //
    HWREG(base + LIN_O_MBRSR) = (uint32_t)(systemClock / 20000U);
}
...
LIN_setMessageFiltering(uint32_t base, LIN_MessageFilter type)
{
    ...
    {
        HWREG(base + LIN_O_SCIGCR1) &= ~LIN_SCIGCR1_HGENCTRL;
    }
    else
    {
        HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_HGENCTRL;
...
LIN_enableParity(uint32_t base)
{
    ...
    //
    HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_PARITYENA;
}
...
LIN_generateParityID(uint32_t identifier)
{

    uint32_t p0, p1, parityIdentifier;
...
//*****************************************************************************
static inline void
LIN_setIDByte(uint32_t base, uint32_t identifier)
{
    //
    // Check the arguments.
    //
    ...
    HWREG(base + LIN_O_ID) = (HWREG(base + LIN_O_ID) & ~(LIN_ID_IDBYTE_M)) |
...
//*****************************************************************************
static inline void
LIN_setIDRESPONDERTask(uint32_t base, uint32_t identifier)
{
    //
    // Check the arguments.
    //
    ...
    HWREG(base + LIN_O_ID) = ((HWREG(base + LIN_O_ID) &
                               ~(LIN_ID_IDRESPONDERTASKBYTE_M)) |
                              ((identifier & LIN_ID_IDBYTE_M) <<
                               LIN_ID_IDRESPONDERTASKBYTE_S));
}
...
LIN_sendWakeupSignal(uint32_t base)
{
    ...
    HWREG(base + LIN_O_TD0) = (HWREG(base + LIN_O_TD0)|(uint32_t)LIN_WAKEUP_KEY<<24);
    //
    // Transmit TDO for wakeup
    //
    HWREG(base + LIN_O_SCIGCR2) |= LIN_SCIGCR2_GENWU;
}
...
LIN_enterSleep(uint32_t base)
{
    ...
    HWREG(base + LIN_O_SCIGCR2) |= LIN_SCIGCR2_POWERDOWN;
}
...
//    HWREG_BP(base + LIN_O_SCIGCR2) |= LIN_SCIGCR2_SC;
	HWREG(base + LIN_O_SCIGCR2) |= LIN_SCIGCR2_SC;
}
...
//    HWREG_BP(base + LIN_O_SCIGCR2) |= LIN_SCIGCR2_CC;
    HWREG(base + LIN_O_SCIGCR2) |= LIN_SCIGCR2_CC;
}
...
LIN_isTxReady(uint32_t base)
{
    ...
    //
    return((HWREG(base + LIN_O_SCIFLR) & LIN_SCIFLR_TXRDY) ==
           LIN_SCIFLR_TXRDY);
}
...
//*****************************************************************************
static inline void
LIN_setFrameLength(uint32_t base, uint32_t length)
{
    //
    // Check the arguments.
    //
    ...
    //
    HWREG(base + LIN_O_SCIFORMAT) &= ~(LIN_SCIFORMAT_LENGTH_M);
    HWREG(base + LIN_O_SCIFORMAT) |= ((((uint32_t)length - (uint32_t)1U) <<
...
LIN_setCommMode(uint32_t base, LIN_CommMode mode)
{
    ...
    //
    HWREG(base + LIN_O_SCIGCR1) = (HWREG(base + LIN_O_SCIGCR1) &
                                    ~LIN_SCIGCR1_COMMMODE) | (uint32_t)mode;
}
...
//*****************************************************************************
static inline void
LIN_setTxMask(uint32_t base, uint32_t mask)
{
    //
    // Check the arguments.
    //
    ...
    //
    HWREG(base + LIN_O_MASK) &= ~(LIN_MASK_TXIDMASK_M);
    HWREG(base + LIN_O_MASK) |= (mask & LIN_MASK_TXIDMASK_M);
}
...
//*****************************************************************************
static inline void
LIN_setRxMask(uint32_t base, uint32_t mask)
{
    //
    // Check the arguments.
    //
    ...
    // Clear previous mask value and set new mask
    //
    HWREG(base + LIN_O_MASK) &= ~(LIN_MASK_RXIDMASK_M);
    HWREG(base + LIN_O_MASK) |= ((uint32_t)mask << LIN_MASK_RXIDMASK_S) &
                                   LIN_MASK_RXIDMASK_M;
}
...
LIN_getTxMask(uint32_t base)
{
    ...
    return(HWREG(base + LIN_O_MASK) & LIN_MASK_TXIDMASK_M);
}
...
LIN_getRxMask(uint32_t base)
{
    ...
    return((uint32_t)((HWREG(base + LIN_O_MASK) & LIN_MASK_RXIDMASK_M) >>
                      LIN_MASK_RXIDMASK_S));
}
...
LIN_isRxReady(uint32_t base)
{
    ...
    //
    return((HWREG(base + LIN_O_SCIFLR) & LIN_SCIFLR_RXRDY) ==
           LIN_SCIFLR_RXRDY);
}
...
LIN_getRxIdentifier(uint32_t base)
{
    ...
    return((uint32_t)((HWREG(base + LIN_O_ID) & LIN_ID_RECEIVEDID_M) >>
                      LIN_ID_RECEIVEDID_S));
}
...
LIN_isTxMatch(uint32_t base)
{
    ...
    //
    return((HWREG(base + LIN_O_SCIFLR) & LIN_SCIFLR_IDTXFLAG) ==
           LIN_SCIFLR_IDTXFLAG);
}
...
LIN_isRxMatch(uint32_t base)
{
    ...
    //
    return((HWREG(base + LIN_O_SCIFLR) & LIN_SCIFLR_IDRXFLAG) ==
           LIN_SCIFLR_IDRXFLAG);
}
...
LIN_enableModuleErrors(uint32_t base, uint32_t errors)
{
    ...
    //
    HWREG(base + LIN_O_IODFTCTRL) &= ~(LIN_IODFTCTRL_IODFTENA_M);

    //
    // Enable write access
    //
    HWREG(base + LIN_O_IODFTCTRL) |= (LIN_IO_DFT_KEY <<
                                       LIN_IODFTCTRL_IODFTENA_S);

    //
    // Enable specified error bits
    //
    HWREG(base + LIN_O_IODFTCTRL) |= errors;
    //
    // Clear the IO DFT Enable Key
    //
    HWREG(base + LIN_O_IODFTCTRL) &= ~(LIN_IO_DFT_KEY <<
...
LIN_enableAutomaticBaudrate(uint32_t base)
{
    ...
    HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_ADAPT;
...
LIN_stopExtendedFrame(uint32_t base)
{
    ...
    // Set stop bit
    //
    HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_STOPEXTFRAME;
}
...
LIN_setChecksumType(uint32_t base, LIN_ChecksumType type)
{
    ...
    {
        HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_CTYPE;
    }
    else
    {
        HWREG(base + LIN_O_SCIGCR1) &= ~(LIN_SCIGCR1_CTYPE);
...
//*****************************************************************************
static inline void
LIN_setSyncFields(uint32_t base, uint32_t syncBreak, uint32_t delimiter)
{
    //
    // Check the arguments.
    //
    ...
    //
    HWREG(base + LIN_O_COMP) &= ~((uint32_t)LIN_COMP_SBREAK_M |
                                   (uint32_t)LIN_COMP_SDEL_M);
    HWREG(base + LIN_O_COMP) |= (syncBreak | ((delimiter - 1U) <<
                                               LIN_COMP_SDEL_S));
}
...
LIN_enableSCIMode(uint32_t base)
{
    ...
    HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_CLK_COMMANDER |
...
LIN_setSCICommMode(uint32_t base, LIN_SCICommMode mode)
{
    ...
    //
    ASSERT(LIN_isBaseValid(base));
    ASSERT((HWREG(base + LIN_O_SCIGCR1) & LIN_SCIGCR1_LINMODE) == 0U);
...
LIN_enableSCIParity(uint32_t base, LIN_SCIParityType parity)
{
    ...
    {
        HWREG(base + LIN_O_SCIGCR1) &= ~(LIN_SCIGCR1_PARITY);
    }
    else
    {
        HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_PARITY;
...
LIN_setSCIStopBits(uint32_t base, LIN_SCIStopBits number)
{
    ...
    {
        HWREG(base + LIN_O_SCIGCR1) &= ~(LIN_SCIGCR1_STOP);
    }
    else
    {
        HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_STOP;
...
LIN_enableSCISleepMode(uint32_t base)
{
    ...
    // Set sleep mode bit
    //
    HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_SLEEP;
}
...
//*****************************************************************************
static inline void
LIN_setSCICharLength(uint32_t base, uint32_t numBits)
{
    //
    // Check the arguments.
    //
    ...
    //
    HWREG(base + LIN_O_SCIFORMAT) = (HWREG(base + LIN_O_SCIFORMAT) &
...
//*****************************************************************************
static inline void
LIN_setSCIFrameLength(uint32_t base, uint32_t length)
{
    //
    // Check the arguments.
    //
...
LIN_readSCICharNonBlocking(uint32_t base, bool emulation)
{
    ...
    //
    return(emulation ? (HWREG(base + LIN_O_SCIED) & LIN_SCIED_ED_M) :
            (HWREG(base + LIN_O_SCIRD) & LIN_SCIRD_RD_M));
}
...
//*****************************************************************************
static inline void
LIN_writeSCICharNonBlocking(uint32_t base, uint32_t data)
{
    //
    // Check the arguments.
    //
    ...
    // Set the Tx Data
    //
    HWREG(base + LIN_O_SCITD) = (data & LIN_SCITD_TD_M);
}
...
//*****************************************************************************
static inline void
LIN_writeSCICharBlocking(uint32_t base, uint32_t data)
{
    //
    // Check the arguments.
    //
...
LIN_isSCIReceiverIdle(uint32_t base)
{
    ...
    //
    return((HWREG(base + LIN_O_SCIFLR) & LIN_SCIFLR_IDLE) == 0U);
}
...
LIN_getSCITxFrameType(uint32_t base)
{
    ...
    //
    return((HWREG(base + LIN_O_SCIFLR) & LIN_SCIFLR_TXWAKE) ==
           LIN_SCIFLR_TXWAKE);
}
...
LIN_getSCIRxFrameType(uint32_t base)
{
    ...
    //
    return((HWREG(base + LIN_O_SCIFLR) & LIN_SCIFLR_RXWAKE) ==
           LIN_SCIFLR_RXWAKE);
}
...
LIN_isSCIBreakDetected(uint32_t base)
{
    ...
    //
    return((HWREG(base + LIN_O_SCIFLR) & LIN_SCIFLR_BRKDT) ==
           LIN_SCIFLR_BRKDT);
}
...
LIN_enableModule(uint32_t base)
{
    ...
    //
    HWREG(base + LIN_O_SCIPIO0) |= (LIN_SCIPIO0_RXFUNC | LIN_SCIPIO0_TXFUNC);
                         ...
                         uint32_t divider)
{
    ...
    //
    HWREG(base + LIN_O_BRSR) = (prescaler | (divider << LIN_BRSR_M_S));
...
LIN_enableDataTransmitter(uint32_t base)
{
    ...
    // Enable transmit bit
    //
    HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_TXENA;
}
...
LIN_enableDataReceiver(uint32_t base)
{
    ...
    // Enable receive bit
    //
    HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_RXENA;
}
...
LIN_isBusBusy(uint32_t base)
{
    ...
    //
    return((HWREG(base + LIN_O_SCIFLR) & LIN_SCIFLR_BUSY) ==
           LIN_SCIFLR_BUSY);
}
...
LIN_isTxBufferEmpty(uint32_t base)
{
    ...
    //
    return((HWREG(base + LIN_O_SCIFLR) & LIN_SCIFLR_TXEMPTY) ==
           LIN_SCIFLR_TXEMPTY);
}
...
LIN_enableIntLoopback(uint32_t base)
{
    ...
    //
    HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_LOOPBACK;
}
...
LIN_getInterruptLevel(uint32_t base)
{
    ...
    //
    return(HWREG(base + LIN_O_SCISETINTLVL));
}
...
LIN_getInterruptLine0Offset(uint32_t base)
{
    ...
    //
    return(HWREG(base + LIN_O_SCIINTVECT0) & LIN_SCIINTVECT0_INTVECT0_M);
}
...
LIN_getInterruptLine1Offset(uint32_t base)
{
    ...
    // Get interrupt vector line offset
    //
    return(HWREG(base + LIN_O_SCIINTVECT1) & LIN_SCIINTVECT1_INTVECT1_M);
}
...
LIN_enableMultibufferMode(uint32_t base)
{
    ...
    HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_MBUFMODE;
}
...
//*****************************************************************************
static inline void
LIN_setTransmitDelay(uint32_t base, uint32_t delay)
{
    //
    // Check the arguments.
    //
    ...
    // Clear delay value
    //
    HWREG(base + LIN_O_IODFTCTRL) &= ~(LIN_IODFTCTRL_TXSHIFT_M);

    //
    // Set the delay value
    //
    HWREG(base + LIN_O_IODFTCTRL) |= ((uint32_t)delay <<
...
LIN_setPinSampleMask(uint32_t base, LIN_PinSampleMask mask)
{
    ...
    // Clear pin mask
    //
    HWREG(base + LIN_O_IODFTCTRL) &= ~(LIN_IODFTCTRL_PINSAMPLEMASK_M);

    //
    // Set new pin mask value
    //
    HWREG(base + LIN_O_IODFTCTRL) |= ((uint32_t)mask <<
...
LIN_setDebugSuspendMode(uint32_t base, LIN_DebugMode mode)
{
    ...
    {
        HWREG(base + LIN_O_SCIGCR1) &= ~(LIN_SCIGCR1_CONT);
    }
    else
    {
        HWREG(base + LIN_O_SCIGCR1) |= LIN_SCIGCR1_CONT;
...
LIN_enableGlobalInterrupt(uint32_t base, LIN_InterruptLine line)
{
    ...
    HWREG(base + LIN_O_GLB_INT_EN) |= LIN_GLB_INT_EN_GLBINT0_EN <<
                                      (uint32_t)line;
}
...
LIN_clearGlobalInterruptStatus(uint32_t base, LIN_InterruptLine line)
{
    ...
    HWREG(base + LIN_O_GLB_INT_CLR) = LIN_GLB_INT_CLR_INT0_FLG_CLR <<
                                        (uint32_t)line;
}
...
LIN_getGlobalInterruptStatus(uint32_t base, LIN_InterruptLine line)
{
    ...
    //
    return((HWREG(base + LIN_O_GLB_INT_FLG) &
            (LIN_GLB_INT_FLG_INT0_FLG << (uint32_t)line)) ==
           (LIN_GLB_INT_FLG_INT0_FLG << (uint32_t)line));
}
...
LIN_getPinStatus(uint32_t base, LIN_PinType pin)
{
    ...
    //
    return((HWREG(base + LIN_O_SCIPIO2) & pin) == pin);
}
