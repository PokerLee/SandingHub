...
typedef enum
{
    XBAR_INPUT1 = 0x0,            //!< ePWM[TZ1], ePWM[TRIP1], X-BARs, eCAPs
    XBAR_INPUT2 = 0x4,            //!< ePWM[TZ2], ePWM[TRIP2], X-BARs, eCAPs
    XBAR_INPUT3 = 0x8,            //!< ePWM[TZ3], ePWM[TRIP3], X-BARs, eCAPs
    XBAR_INPUT4 = 0xC,            //!< ADC wrappers, X-BARs, XINT1, eCAPs
    XBAR_INPUT5 = 0x10,            //!< EXTSYNCIN1, X-BARs, XINT2, eCAPs
    XBAR_INPUT6 = 0x14,            //!< EXTSYNCIN2, ePWM[TRIP6], X-BARs, XINT3, eCAPs
    XBAR_INPUT7 = 0x18,            //!< X-BARs, eCAPs
    XBAR_INPUT8 = 0x1C,            //!< X-BARs, eCAPs
    XBAR_INPUT9 = 0x20,            //!< X-BARs, eCAPs
    XBAR_INPUT10 = 0x24,           //!< X-BARs, eCAPs
    XBAR_INPUT11 = 0x28,           //!< X-BARs, eCAPs
    XBAR_INPUT12 = 0x2C,           //!< X-BARs, eCAPs
    XBAR_INPUT13 = 0x30,           //!< XINT4, X-BARs, eCAPs
    XBAR_INPUT14 = 0x34,           //!< XINT5, X-BARs, eCAPs
    XBAR_INPUT15 = 0x38,           //!< eCAPs
    XBAR_INPUT16  = 0x3C         //!< eCAPs
} XBAR_InputNum;
...
{
    uint32_t outputNum = (uint32_t)output;
    //
    // Check the arguments.
    //
...
XBAR_setOutputLatchMode(uint32_t base, XBAR_OutputNum output, bool enable)
{
    ...
    if(enable)
    {
        HWREG(base + XBAR_O_OUTPUTLATCHENABLE) |=
               0x1U << ((uint32_t)output / 4U);
    }
    else
    {
        HWREG(base + XBAR_O_OUTPUTLATCHENABLE) &=
...
XBAR_getOutputLatchStatus(uint32_t base, XBAR_OutputNum output)
{
    ...
    //
    return((HWREG(base + XBAR_O_OUTPUTLATCH) &
            (0x1U << ((uint32_t)output / 4U))) != 0U);
}
...
XBAR_clearOutputLatch(uint32_t base, XBAR_OutputNum output)
{
    ...
    //
        HWREG(base + XBAR_O_OUTPUTLATCHCLR) |=
        0x1U << ((uint32_t)output / 4U);
}
...
XBAR_forceOutputLatch(uint32_t base, XBAR_OutputNum output)
{
    ...
    //
    HWREG(base + XBAR_O_OUTPUTLATCHFRC) =
        (uint32_t)0x1U << ((uint32_t)output / 4U);
}
...
XBAR_invertEPWMSignal(XBAR_TripNum trip, bool invert)
{
    ...
    if(invert)
    {
        HWREG(EPWMXBAR_BASE + XBAR_O_TRIPOUTINV) |=
            0x1U << ((uint32_t)trip / 4U);
    }
    else
    {
        HWREG(EPWMXBAR_BASE + XBAR_O_TRIPOUTINV) &=
...
//*****************************************************************************
static inline void
XBAR_setInputPin(uint32_t base, XBAR_InputNum input, uint32_t pin)
{
    //
    // Check the argument.
    //
    ...
    HWREG(base + XBAR_O_INPUT1SELECT + (uint32_t)input) = pin;
...
XBAR_invertCLBSignal(XBAR_AuxSigNum auxSignal, bool invert)
{
    ...
    if(invert)
    {
        HWREG(CLBXBAR_BASE + XBAR_O_AUXSIGOUTINV) |=
            0x1U << ((uint32_t)auxSignal / 4U);
    }
    else
    {
        HWREG(CLBXBAR_BASE + XBAR_O_AUXSIGOUTINV) &=
