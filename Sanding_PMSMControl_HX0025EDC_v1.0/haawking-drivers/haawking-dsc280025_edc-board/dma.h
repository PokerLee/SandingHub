...
typedef enum
{
	...
	DMA_TRIGGER_SCIA_TX    = 192,
	DMA_TRIGGER_SCIA_RX    = 193,

	DMA_TRIGGER_I2CA_TX    = 194,
	DMA_TRIGGER_I2CA_RX    = 195,

	DMA_TRIGGER_I2CB_TX    = 196,
	DMA_TRIGGER_I2CB_RX    = 197
} DMA_Trigger;
...
DMA_initController(void)
{
    ...
    //
    HWREG(DMA_BASE + DMA_O_CTRL) |= DMA_CTRL_HARDRESET;
    NOP;
...
DMA_triggerSoftReset(uint32_t base)
{
    ...
    //
    HWREG(base + DMA_O_CONTROL) |= DMA_CONTROL_SOFTRESET;
    NOP;
...
DMA_setEmulationMode(DMA_EmulationMode mode)
{
    ...
    {
        HWREG(DMA_BASE + DMA_O_DEBUGCTRL) &= ~DMA_DEBUGCTRL_FREE;
    }
    else
    {
        HWREG(DMA_BASE + DMA_O_DEBUGCTRL) |= DMA_DEBUGCTRL_FREE;
...
DMA_enableTrigger(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + DMA_O_MODE) |= DMA_MODE_PERINTE;
    EDIS;
}
...
DMA_forceTrigger(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + DMA_O_CONTROL) |= DMA_CONTROL_PERINTFRC;
    EDIS;
}
...
DMA_clearTriggerFlag(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + DMA_O_CONTROL) |= DMA_CONTROL_PERINTCLR;
    EDIS;
}
...
DMA_getTransferStatusFlag(uint32_t base)
{
    ...
    //
    return((HWREG(base + DMA_O_CONTROL) & DMA_CONTROL_TRANSFERSTS) != 0U);
}
...
DMA_getBurstStatusFlag(uint32_t base)
{
    ...
    //
    return((HWREG(base + DMA_O_CONTROL) & DMA_CONTROL_BURSTSTS) != 0U);
}
...
DMA_getRunStatusFlag(uint32_t base)
{
    ...
    //
    return((HWREG(base + DMA_O_CONTROL) & DMA_CONTROL_RUNSTS) != 0U);
}
...
DMA_getOverflowFlag(uint32_t base)
{
    ...
    //
    return((HWREG(base + DMA_O_CONTROL) & DMA_CONTROL_OVRFLG) != 0U);
}
...
DMA_getTriggerFlagStatus(uint32_t base)
{
    ...
    //
    return((HWREG(base + DMA_O_CONTROL) & DMA_CONTROL_PERINTFLG) != 0U);
}
...
DMA_startChannel(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + DMA_O_CONTROL) |= DMA_CONTROL_RUN;
    EDIS;
}
...
DMA_stopChannel(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + DMA_O_CONTROL) |= DMA_CONTROL_HALT;
    EDIS;
}
...
DMA_enableInterrupt(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + DMA_O_MODE) |= DMA_MODE_CHINTE;
    EDIS;
}
...
DMA_enableOverrunInterrupt(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + DMA_O_MODE) |= DMA_MODE_OVRINTE;
    EDIS;
}
...
DMA_clearErrorFlag(uint32_t base)
{
    ...
    //
    EALLOW;
    HWREG(base + DMA_O_CONTROL) |= DMA_CONTROL_ERRCLR;
    EDIS;
}
...
DMA_setInterruptMode(uint32_t base, DMA_InterruptMode mode)
{
    ...
    {
        HWREG(base + DMA_O_MODE) |= DMA_MODE_CHINTMODE;
    }
    else
    {
        HWREG(base + DMA_O_MODE) &= ~DMA_MODE_CHINTMODE;
...
DMA_setPriorityMode(bool ch1IsHighPri)
{
    ...
    if(ch1IsHighPri)
    {
        HWREG(DMA_BASE + DMA_O_PRIORITYCTRL1) |=
...
//
//*****************************************************************************
extern void DMA_configBurst(uint32_t base, uint32_t size, int32_t srcStep,
                            int16_t destStep);
